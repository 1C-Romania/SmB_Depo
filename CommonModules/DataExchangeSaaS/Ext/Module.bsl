						...
						If Selection2.Next() Then
							...
							If CorrespondentEndPoint = Undefined Then
								...
								Raise StringFunctionsClientServer.PlaceParametersIntoString(
									NStr("en = 'The correspondent end point is not defined for the address ""%1"".'"),
									Selection2.Address);
							EndIf;
...
Function InformationExchangeDirectoryRelative(Val Folder)
	...
	If IsBlankString(Folder) Then
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en = 'Incorrect info exchange catalog format ""%1"".'"),
			Folder);
	EndIf;
...
Procedure ExecuteDataExchangeScenarioActionInFirstInfobase(ScenarioRowIndex, DataExchangeScenario) Export
	...
	If ScenarioString.InfobaseNumber = 1 Then
		...
		Else
			Raise StringFunctionsClientServer.PlaceParametersIntoString(
				NStr("en = 'Unknown action ""%1"" when exchanging data between data areas.'"),
...
Function GeneratePredefinedNodeDescription() Export
	...
	Return ?(IsBlankString(ApplicationName), NStr("en = 'Application in Internet'"), ApplicationName);
EndFunction
...
Function CorrespondentEndPoint(Val Correspondent) Export
	...
	If QueryResult.IsEmpty() Then
		MessageString = NStr("en = 'The correspondent end point is not specified for ""%1"".'");
...
Procedure AvailableNewData(Val Handle, Import) Export
	...
	If Handle.DataType = ProvidedDataKindId() Then
		...
		Else
			...
			MessageText = NStr("en = 'Provided exchange rules are not applicable for the current configuration and intended for the %1 exchange plan of the %3 version of the %2 configuration'");
			...
			WriteLogEvent(NStr("en = 'Provided data exchange rules. The loading of the provided rules is cancelled.'",
...
Procedure CreateExchangeSetting(
	...
	Try
		...
		If IsBlankString(ThisNodeCode) Then
			...
			MessageString = NStr("en = 'For the predefined exchange plan node ""%1"" the code is not specified.'");
		...
		If IsBlankString(GetFunctionalOption("InfobasePrefix")) Then
			...
			If IsBlankString(Prefix) Then
				Raise NStr("en = 'In the service Manager the prefix for this application is not installed.'");
			EndIf;
		...
		If TransportSettings.ExchangeMessageTransportKindByDefault = Enums.ExchangeMessagesTransportKinds.FILE Then
			...
			If IsBlankString(FILEInformationExchangeCommonDirectory) Then
				...
				MessageString = NStr("en = 'Info exchange directory is not specified for the end point ""%1"".'");
			...
			If Not CommonDirectory.Exist() Then
				...
				MessageString = NStr("en = 'Info exchange directory ""%1"" does not exist.'");
		...
		Else
			Raise StringFunctionsClientServer.PlaceParametersIntoString(
				NStr("en = 'Exchange message transport kind ""%1"" is not supported for the %2 end point.'"),
...
Procedure SaveSessionData(Val Message, Val Presentation = "") Export
	...
	If Not IsBlankString(Presentation) Then
		Presentation = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en = ' {%1}'"), Presentation);
	EndIf;
	
	MessageString = NStr("en = 'System messages exchange session ""%1"" is successfully completed.%2'",
...
Procedure FixUnsuccessfullSessionCompletion(Val Message, Val Presentation = "") Export
	...
	MessageString = NStr("en = 'Error of the %1 system message
		|exchange session performance. %2 Error description from the correspondent: %3'", CommonUseClientServer.MainLanguageCode());
	MessageString = StringFunctionsClientServer.PlaceParametersIntoString(MessageString,
...
Function EventLogMonitorEventDataSyncronizationSetting() Export
	...
	Return NStr("en = 'Data exchange in the service model. Data synchronization setup'",
		CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorEventDataSynchronizationMonitor() Export
	...
	Return NStr("en = 'Data exchange in the service model. Data synchronization monitor'",
		CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorEventDataSynchronization() Export
	...
	Return NStr("en = 'Data exchange in the service model. Data synchronization'",
		CommonUseClientServer.MainLanguageCode());
...
Function SystemMessagesExchangeSessionEventLogMonitorEvent()
	...
	Return NStr("en = 'Data exchange in the service model. Sessions of the system messages exchange'",
		CommonUseClientServer.MainLanguageCode());
...
Function FindInfobaseNode(Val ExchangePlanName, Val NodeCode)
	...
	If Result = Undefined Then
		Message = NStr("en = 'The exchange plan node is not found. Name of the %1 exchange plan; code of the %2 or %3 node'");
