...
Function GetDatesArrayByCalendar(Val WorkSchedule, Val DateFrom, Val DaysArray, Val CalculateNextDateFromPrevious = False, CallingException = True) Export
	...
	While Selection.Next() Do
		If Selection.DateInCalendar = Undefined Then
			ErrorInfo = NStr("en = 'Business calendar ""%1"" is not filled in from date %2 for the specified number of business days.'");
...
Function GetDatesDiffByCalendar(Val WorkSchedule, Val StartDate, Val EndDate, CallingException = True) Export
	...
	If Result.IsEmpty() Then
		If CallingException Then
			ErrorInfo = NStr("en = 'Work schedule ""%1"" is not filled for the period %2.'");
			Raise StringFunctionsClientServer.PlaceParametersIntoString(
				ErrorInfo,
		...
		Else 
			...
			If CallingException Then
				ErrorMessage = StringFunctionsClientServer.PlaceParametersIntoString(
									NStr("en = 'Unable to define the closest work date for the %1 date, perhaps, the work schedule is not filled.'"), 
...
Function SchedulesOfWorkOnPeriod(Graphics, StartDate, EndDate) Export
	...
	Raise NStr("en = 'Subsystem ""Work schedules"" is not found.'");
