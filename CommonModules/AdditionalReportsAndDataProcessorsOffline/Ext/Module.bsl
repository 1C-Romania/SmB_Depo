...
Procedure AdditionalProcessingBeforeWrite(Source, Cancel) Export
	...
	If OfflineWorkService.ThisIsOfflineWorkplace() Then
		...
		|Для удаления дополнительного отчета или обработки необходимо
		|выполнить операцию отключения в приложении сервиса и провести синхронизацию данных автономного рабочего места с сервисом.'"),
				Source.Description);
