...
Procedure ImportRates() Export
	...
	If Descriptors.Descriptor.Count() < 1 Then
		Raise(NStr("en = 'There is no data of kind ""ExchangeRates"" in the service manager.'"));
	EndIf;
...
Procedure ProcessProvidedRatesForDay(Val Handle, Val PathToFile)
	...
	If CurrencyRatesDate = "" Then
		Raise NStr("en = 'Data of kind ""ExchangeRatesForDay"" does not contain characteristic ""Date"". Exchange rate update is impossible.'"); 
	EndIf;
