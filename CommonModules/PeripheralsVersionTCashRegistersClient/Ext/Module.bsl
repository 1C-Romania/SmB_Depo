...
Function ConnectDevice(DriverObject, Parameters, ConnectionParameters, Output_Parameters) Export
	...
	Result = True;
	ConnectionParameters.Insert("CashRegisterRegistrationNumber", "0");
	...
	Parameters.Property("LogDriver"    , LogDriver);
	...
	If Port                       = Undefined
	 ...
	 Or LogDriver     = Undefined Then
		...
		Result = False;
	EndIf;
	// End device parameters checking.
	...
	If Result Then
		...
		ValueArray.Add(Parameters.NetworkNumber);
		ValueArray.Add(Parameters.DevicePassword);
		ValueArray.Add(Parameters.Timeout);
		ValueArray.Add(String(Parameters.UserPassword));
		ValueArray.Add(String(Parameters.AdministratorPassword));
		ValueArray.Add(ConnectionParameters.CashRegisterRegistrationNumber);
		ValueArray.Add(ConnectionParameters.CashRegisterSerialNumber);
		ValueArray.Add(mVAT[0]);
		ValueArray.Add(mVAT[1]);
		ValueArray.Add(mVAT[2]);
		ValueArray.Add(mVAT[3]);
		ValueArray.Add(Parameters.PaymentDescription1);
		ValueArray.Add(Parameters.PaymentDescription2);
		ValueArray.Add(Parameters.LogDriver);
		...
		If Not DriverObject.Connect(ValueArray, ConnectionParameters.DeviceID) Then
			...
			Result = False;
		Else
			ConnectionParameters.CashRegisterRegistrationNumber = ValueArray[7];
			ConnectionParameters.CashRegisterSerialNumber        = ValueArray[8];
...
Function ReceiptPrint(DriverObject, Parameters, ConnectionParameters, InputParameters, Output_Parameters)
	...
	Return EquipmentManagerClientOverridable.ReceiptPrint(PeripheralsVersionTCashRegistersClient,
...
Function DeviceTest(DriverObject, Parameters, ConnectionParameters, Output_Parameters)
	...
	CashRegisterRegistrationNumber = "0";
	CashRegisterSerialNumber = "0";
