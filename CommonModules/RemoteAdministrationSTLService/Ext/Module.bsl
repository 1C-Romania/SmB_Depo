...
Procedure DataAreaSessionEnd(Val SessionNumber, Val UserPassword) Export
	...
	Else
		...
		Raise NStr("en = 'Current version of managing application does not support finishing of the session from applications. It is required to update the managing application.'");
...
Procedure SendQueriesForExternalResourcesUse(Val RequestsSerialization) Export
	...
	Else
		...
		Raise NStr("en = 'Current version of managing application does not support requests for permissions to use external resources. It is required to update the managing application.'");
