...
Procedure CancelJobExecution(Val JobID) Export 
	...
	Except
		...
		WriteLogEvent(NStr("en = 'Long actions. Background job execution cancellation'", CommonUseClientServer.MainLanguageCode()),
...
Function JobCompleted(Val JobID) Export
	...
	If Task = Undefined Then
		WriteLogEvent(NStr("en = 'Long actions. Background job has not been found'", CommonUseClientServer.MainLanguageCode()),
	...
	Else
		...
		ElsIf Task.State = BackgroundJobState.Canceled Then
			WriteLogEvent(
				NStr("en = 'Long actions. Background task has been cancelled by administrator'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				,
				NStr("en = 'Job has been completed with an unknown error.'"));
		Else
			Return True;
		EndIf;
	EndIf;
...
Procedure ExecuteProcedureDataProcessorsObjectModule(Parameters, StorageAddress) Export 
	...
	Except
		WriteLogEvent(NStr("en = 'Safe execution of the processor method'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error, , , DetailErrorDescription(ErrorInfo()));
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en='Method name ""%1"" does not correspond to the requirements of the variable names formation.'"),
			MethodName);
	EndTry;
