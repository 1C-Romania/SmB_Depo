...
Function ConnectDevice(DriverObject, Parameters, ConnectionParameters, Output_Parameters) Export
	...
	ImageAtStart		   = Undefined;
	ImageAtEnd          = Undefined;
	ImageAfter     	   = Undefined;
	CashRegisterSerialNumber		   = Undefined;
	CashRegisterRegistrationNumber    = Undefined;
	PaymentByCash             = Undefined;
	...
	Parameters.Property("ImageAtEnd" 			, ImageAtEnd);
	Parameters.Property("ImageAtStart" 		, ImageAtStart);
	Parameters.Property("ImageAfter" 			, ImageAfter);
	...
	Parameters.Property("PaymentByCash" 			, PaymentByCash);
	...
	If Port                      = Undefined           
	 ...
	 Or AdministratorPassword      = Undefined
	 Or PaymentDescription1       = Undefined
	 Or PaymentDescription2       = Undefined 
	 Or ImageAtEnd         = Undefined
	 Or ImageAtStart        = Undefined
	 Or ImageAfter          = Undefined
	 Or SectionNumber               = Undefined
	 Or PaymentByCash            = Undefined
	 Or PaymentByCash            = Undefined 
	 Then
		...
		Result = False;
	EndIf;
	// End: Checks device parameters.

	If Result Then
		ValueArray = New Array;
		ValueArray.Add(Port);
		ValueArray.Add(Speed);
		ValueArray.Add(UserPassword);
		ValueArray.Add(AdministratorPassword);
		ValueArray.Add(Timeout);
		ValueArray.Add(CashRegisterRegistrationNumber);
		ValueArray.Add(CashRegisterSerialNumber);
		ValueArray.Add(PaymentDescription1);
		ValueArray.Add(PaymentDescription2);
		ValueArray.Add(ImageAtStart);
		ValueArray.Add(ImageAtEnd);
		ValueArray.Add(ImageAfter);
...
Function ReceiptPrint(DriverObject, Parameters, ConnectionParameters, InputParameters, Output_Parameters)
	...
	Return EquipmentManagerClientOverridable.ReceiptPrint(PeripheralsORIONFiscalRegistersClient,
...
Function PrintBarcode(DriverObject, Parameters, ConnectionParameters, BarCodeType, Barcode, Output_Parameters)
	...
	TextString = NStr("en='Barcode:'") + Barcode; 
	Result = DriverObject.PrintNonFiscalLine(ConnectionParameters.DeviceID, TextString);
	If Not Result Then
		Output_Parameters.Clear();
		Output_Parameters.Add(999);
...
Function OpenCashDrawer(DriverObject, Parameters, ConnectionParameters, Output_Parameters)
	...
	Except
		Output_Parameters.Add(999);
		Output_Parameters.Add(NStr("en='The ""Open cash drawer"" command is not supported by this driver.'"));
		Return Result;
	EndTry;
...
// Function carries out Device Test
Function DeviceTest(DriverObject, Parameters, ConnectionParameters, Output_Parameters)
	...
	ValueArray.Add(Parameters.UserPassword);
	ValueArray.Add(Parameters.AdministratorPassword);
	ValueArray.Add(Parameters.Timeout);
	ValueArray.Add(CashRegisterRegistrationNumber);
	ValueArray.Add(CashRegisterSerialNumber);
	ValueArray.Add(Parameters.PaymentDescription1);
	ValueArray.Add(Parameters.PaymentDescription2);
	ValueArray.Add(Parameters.ImageAtStart);
	ValueArray.Add(Parameters.ImageAtEnd);
	ValueArray.Add(Parameters.ImageAfter);     
