...
Function SendSMS(RecipientNumbers, Text, SenderName, Login, Password) Export
	...
	If IsBlankString(RecipientsString) Or IsBlankString(Text) Then
		Result.ErrorDescription = NStr("en = 'Incorrect message parameters'");
		Return Result;
	EndIf;
	...
	If IsBlankString(FileNameResponse) Then
		Result.ErrorDescription = Result.ErrorDescription + NStr("en = 'Connection is not set'");
		Return Result;
	EndIf;		
...
Function permissions() Export
	...
	Port = Undefined;
	Definition = NStr("en = 'Sending SMS by Beeline.'");
