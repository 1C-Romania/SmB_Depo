...
Function ExternalComponentsKitFilesControlSums(Val TemplateName) Export
	...
	Else
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en = 'Failed to generate a permission to use
				  |an external component: incorrect template name %1!'"), TemplateName);
	EndIf;
	
	If Template = Undefined Then
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en = 'Failed to generate the permission to use
				  |the external component delivered with the template 1%: template% 1 is not found in the configuration content!'"), TemplateName);
	EndIf;
	...
	If Metadata.FindByFullName(TemplateName).TemplateType <> Metadata.ObjectProperties.TemplateType.BinaryData Then
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en = 'Failed to generate a permission for the
				  |use of the external component: template %1 does not contain binary data!'"), TemplateName);
	EndIf;
	...
	If IsBlankString(ManifestFile) Then
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en = 'Failed to generate the permission to use
				  |the external component delivered with the template %1: file MANIFEST.XML is not found in the archive!'"), TemplateName);
	EndIf;
...
Procedure OnWriteServiceData(Object) Export
	...
	If WorkInSafeMode.SafeModeIsSet() Then
		...
		Raise StringFunctionsClientServer.PlaceParametersIntoString(
			NStr("en = 'Record of object %1 is not available: set safe mode: %2!'"),
