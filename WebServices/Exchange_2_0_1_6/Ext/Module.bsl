...
Function GetFilePart(TransferId, PartNumber, PartData)
	...
	If FileNames.Count() = 0 Then
		...
		MessagePattern = NStr("en = 'Fragment %1 of transfer session with the %2 ID is not found'");
	...
	ElsIf FileNames.Count() > 1 Then
		...
		MessagePattern = NStr("en = 'Several fragments %1 of the transfer session with the %2 ID have been found'");
		MessageText = StringFunctionsClientServer.PlaceParametersIntoString(MessagePattern, String(PartNumber), String(TransferId));
		Raise(MessageText);
...
Function CheckConnection(ExchangePlanName, NodeCode, Result)
	...
	If ExchangePlans[ExchangePlanName].FindByCode(NodeCode).IsEmpty() Then
		Result = NStr("en = 'Possibility of the data synchronization is disabled by the application administrator in the Internet.'");
		Return False;
	EndIf;
...
Procedure ValidateLockInformationBaseForUpdate()
	...
	If ValueIsFilled(InfobaseUpdateService.InfobaseLockedForUpdate()) Then
		...
		Raise NStr("en = 'Data synchronization is temporarily unavailable due to the application updating in the Internet.'");
...
Procedure ExecuteDataExportInClientServerMode(ExchangePlanName,
	...
	If BackgroundJobs.GetBackgroundJobs (Filter).Count() = 1 Then
		Raise NStr("en = 'Data synchronization is already executing.'");
	EndIf;
	
	BackgroundJob = BackgroundJobs.Execute("DataExchangeServer.ExportToFileTransferServiceForInfobaseNode",
										Parameters,
										BackgroundJobKey,
										NStr("en = 'Data exchange through the web service is being performed.'"));
	...
	If BackgroundJob.State <> BackgroundJobState.Completed Then
		...
		Raise NStr("en = 'Error when exporting the data through the web service.'");
	EndIf;
...
Procedure ImportDataInClientServerMode(ExchangePlanName,
	...
	If BackgroundJob.State <> BackgroundJobState.Completed Then
		...
		Raise NStr("en = 'Error when importing the data through the web service.'");
	EndIf;
...
Function RegisterDataForInitialExport(Val ExchangePlanName, Val NodeCode, LongOperation, ActionID, CatalogsOnly)
	...
	Else
		...
		BackgroundJob = BackgroundJobs.Execute(MethodName, Parameters,, NStr("en = 'Data exchange creation.'"));
...
Function FindFileOfPart(Val Directory, Val FileNumber)
	...
	For CountNumberOfBits = CountNumberOfBitsCiesla(FileNumber) To 5 Do
		...
		FormatString = StringFunctionsClientServer.PlaceParametersIntoString("ND=%1; NLZ=; NG=0", String(CountNumberOfBits));
		
		FileName = StringFunctionsClientServer.PlaceParametersIntoString("data.zip.%1", Format(FileNumber, FormatString));
