...
Function PrintForm(ObjectsArray, PrintObjects, TemplateName)
	...
	For Each CurrentDocument IN ObjectsArray Do
		...
		ElsIf TemplateName = "TORG13" Then
			...
			|	DocumentHeader.StructuralUnit AS Sender,
			|	DocumentHeader.StructuralUnitPayee AS Recipient,
			|	DocumentHeader.GLExpenseAccount.Code AS SubAccount,
			|	DocumentHeader.Number AS Number,
			|	DocumentHeader.Company.Prefix AS Prefix,
			|	DocumentHeader.Released AS Released,
			|	DocumentHeader.ReleasedPosition AS ReleasedPosition
			|FROM
			|	Document.InventoryTransfer AS DocumentHeader
			|WHERE
			|	DocumentHeader.Ref = &CurrentDocument
			...
			|	InventoryTransfer.Characteristic AS Characteristic,
			|	InventoryTransfer.ProductsAndServices.InventoryGLAccount.Code AS Account,
			|	InventoryTransfer.ProductsAndServices.Code AS ProductCode,
			|	InventoryTransfer.MeasurementUnit AS MeasurementUnit,
			|	InventoryTransfer.ProductsAndServices.MeasurementUnit.Description AS MeasurementUnitDescription,
			|	InventoryTransfer.ProductsAndServices.MeasurementUnit.Code AS MeasurementUnitCodeByOKEI
			|INTO DocumentTable
			|FROM
			|	Document.InventoryTransfer.Inventory AS InventoryTransfer
			|WHERE
			...
			|	DocumentTable.InventoryItem,
			|	DocumentTable.SKU,
			|	DocumentTable.Quantity AS PlacesQuantity,
			|	CASE
			|		WHEN DocumentTable.MeasurementUnit REFS Catalog.UOM
			|			THEN DocumentTable.MeasurementUnit.Factor
			|		ELSE 1
			...
			|	DocumentTable.Characteristic,
			|	DocumentTable.Account,
			|	DocumentTable.ProductCode,
			|	DocumentTable.MeasurementUnitDescription,
			|	DocumentTable.MeasurementUnitCodeByOKEI,
			|	CASE
			|		WHEN &CurrencyPriceKind = NationalCurrency.Value
			|			THEN ISNULL(ProductsAndServicesPricesSliceLast.Price, 0)
			|		ELSE ISNULL(ProductsAndServicesPricesSliceLast.Price, 0) * CurrencyRatesSliceLast.ExchangeRate / CurrencyRatesSliceLast.Multiplicity
			...
			SpreadsheetDocument.PrintParametersName = "PRINT_PARAMETERS_InventoryTransfer_TRAD13";
			
			Template = PrintManagement.PrintedFormsTemplate("Document.InventoryTransfer.PF_MXL_TORG13");
			...
			While LinesSelectionInventory.Next() Do
				...
				If (AWholePage = Int(AWholePage))
				 OR ((WrapLastRow = 1) AND (Num = LineCount)) Then
					...
					TotalsAreaByPage.Parameters.TotalPlacesCountByPage = TotalPlacesCountByPage;
					TotalsAreaByPage.Parameters.TotalAmountPerPage = TotalAmountPerPage;
				...
				TemplateArea.Parameters.Fill(LinesSelectionInventory);
				PlacesQuantity = LinesSelectionInventory.PlacesQuantity;
				TemplateArea.Parameters.ProductDescription = SmallBusinessServer.GetProductsAndServicesPresentationForPrinting(
					LinesSelectionInventory.InventoryItem,
					LinesSelectionInventory.Characteristic,
					LinesSelectionInventory.SKU);
				SpreadsheetDocument.Put(TemplateArea);
				...
				TotalAmountPerPage = TotalAmountPerPage + LinesSelectionInventory.Amount;
			...
			TemplateArea = Template.GetArea("Total");
			TemplateArea.Parameters.TotalQuantityOfPlaces = TotalQuantity;
			TemplateArea.Parameters.TotalAmount = TotalAmount;
		...
		ElsIf TemplateName = "MX18" Then
			...
			|	InventoryTransfer.StructuralUnit AS Sender,
			|	InventoryTransfer.StructuralUnitPayee AS Recipient,
			|	InventoryTransfer.GLExpenseAccount.Code AS SubAccount,
			|	InventoryTransfer.Number,
			|	InventoryTransfer.Company.Prefix AS Prefix,
			|	InventoryTransfer.Released AS Released,
			|	InventoryTransfer.ReleasedPosition AS ReleasedPosition,
			...
			|		CASE
			|			WHEN InventoryTransfer.Inventory.MeasurementUnit REFS Catalog.UOM
			|				THEN InventoryTransfer.Inventory.MeasurementUnit.Factor
			|			ELSE 1
			|		END AS QuantityInOnePlace
			|	)
			|FROM
			...
			SpreadsheetDocument.PrintParametersName = "PRINT_PARAMETERS_InventoryTransfer_MH18";
			
			Template = PrintManagement.PrintedFormsTemplate("Document.InventoryTransfer.PF_MXL_MX18");
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "TORG13") Then
		
		PrintManagement.OutputSpreadsheetDocumentToCollection(PrintFormsCollection, "TORG13", "TORG-13", PrintForm(ObjectsArray, PrintObjects, "TORG13"));
