...
Procedure SendMailing(Command)
	...
	If Write() Then
		...
		NotificationText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Successfully sent: %1 messages';ru='Успешно отправлено: %1 сообщений'"), SuccessfullySent);
...
Procedure CheckEmailAddressCorrectness(Cancel)
	...
	For Each RecipientRow IN Object.Recipients Do
		...
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Recipient email is specified incorrectly: %1, due to: %2';ru='Некорректно указан E-mail получателя: %1, по причине: %2'"),
...
Function SendEmailMailing()
	...
	For Each RecipientRow IN Object.Recipients Do
		...
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Cannot send Email to recipient: %1, due to: %2';ru='Не удалось отправить E-mail получателю: %1, по причине: %2'"),
				RecipientRow.Contact,
				BriefErrorDescription(ErrorInfo()),
				);
...
Function SendSMSMailing()
	...
	For Each RecipientRow IN Object.Recipients Do
		...
		If IsBlankString(SendingResult.ErrorDescription) Then
			...
			SuccessfullySent = SuccessfullySent + 1;
		Else
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Cannot send SMS to recipient: %1, due to: %2';ru='Не удалось отправить SMS получателю: %1, по причине: %2'"),
				RecipientRow.Contact,
