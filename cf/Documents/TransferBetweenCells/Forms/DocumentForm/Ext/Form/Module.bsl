...
Procedure FillByBalanceAtWarehouse()
	...
	|	InventoryInWarehousesOfBalance.ProductsAndServices.Cell AS Cell
	|FROM
...
Procedure SetVisibleFromOperationKind()
	...
	If Object.OperationKind = Enums.OperationKindsTransferBetweenCells.FromOneToSeveral Then
		ThisForm.Items.InventoryCommandFillWithWarehouseInventory.Enabled = True;
	Else
		ThisForm.Items.InventoryCommandFillWithWarehouseInventory.Enabled = False;
	EndIf;
	
EndProcedure // SetVisibleByOperationKind()
...
Procedure FillByBalance(Command)
	...
	If Object.Inventory.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillInByBalanceEnd", ThisObject), NStr("en='Tabular section will be cleared. Continue?';ru='Табличная часть будет очищена! Продолжить выполнение операции?'"), QuestionDialogMode.YesNo, 0);
        Return; 
	EndIf;
