...
//		
//		CashDesk = CommonAtServer.GetUserSettingsValue("DefaultCashDesk");
//		ExchangeRate = CommonAtServer.GetExchangeRate(CashDesk.Currency, Date);
//		
//		
...
//		FillPropertyValues(ThisObject, PurchaseOrder,,"Number, ExchangeRate");
//		Date = GetServerDate();

//		CashDesk = CommonAtServer.GetUserSettingsValue("DefaultCashDesk");
//		ExchangeRate = CommonAtServer.GetExchangeRate(CashDesk.Currency, Date);
//		
//		SettlementCurrency = PurchaseOrder.Currency;
//		SettlementExchangeRate = CommonAtServer.GetExchangeRate(SettlementCurrency, Date);
//		Partner = PurchaseOrder.Supplier;
//		OperationType = Enums.OperationTypesAccountsOutgoing.ForSupplier;
//		SettlementPrepaymentAmount = PurchaseOrder.Amount;
//		Amount = ?(ExchangeRate = 0, 0, PurchaseOrder.Amount * SettlementExchangeRate / ExchangeRate);
//		SettlementAmount = PurchaseOrder.Amount;
//		
//		NewRowDoc = ReservedPrepayments.Add();
//		NewRowDoc.ReservationDocument = PurchaseOrder;
...
//	ElsIf Base = Undefined OR TypeOf(Base) = Type("Structure") Then
...
//				ExchangeRate = CommonAtServer.GetExchangeRate(CashDesk.Currency, Date);
//				SettlementCurrency = CashDesk.Currency;
...
//		ReservedPrepayments.Clear();
//		SettlementAmount = Records.Total("AmountDrNational") - Records.Total("AmountCrNational");
//	Else
//		Records.Clear();
//	EndIf;
//	
...
//				ExchangeRate = CommonAtServer.GetExchangeRate(CashDesk.Currency, BegOfDay(Date) - 1);
//				AmountNational = ExchangeRate * Amount;
//			Else
//				AmountNational = BankCash.GetCashNationalAmountAccordingToAverageExchangeRate(Amount, CashDesk.Currency, Date, CashDesk, Company);
...
//			EndIf;
//			
//			If CashDesk.Currency = SettlementCurrency Then
//				SettlementExchangeRate = ExchangeRate;
//			EndIf;	
//			
//		EndIf;
...
//	CashAmount = BankCash.CheckBankOrCashAmount(Company, CashDesk, CashDesk.Currency);
//	If CashAmount <= 0 Then
//		Alerts.AddAlert(NStr("en='Insufficient funds on the cash desk in order to make payment. Current amount of the cash desk: ';pl='Brak środków w kasie aby dokonać wypłatę. Bieżąca kwota w kasie: ';ru='Недостаточное количество наличных средств в кассе для проведения платежа. Текущая сумма в кассе: '") + " " + Format(CashAmount, "ND=15; NFD=2"), Enums.AlertType.Warning, Cancel,ThisObject);
//	ElsIf CashAmount < Amount Then
//		Alerts.AddAlert(NStr("en='The amount in the cash desk is insufficient for covering the payment. Available amount: ';pl='Kwota w kasie jest niewystarczająca do pokrycia wypłaty. Dostępna kwota: ';ru='Сумма наличных средств в кассе недостаточна для проведения оплаты. Доступная сумма: '") + " " + Format(CashAmount, "ND=15; NFD=2"), Enums.AlertType.Warning, Cancel,ThisObject);
//	EndIf;
//	
//	BankCash.PostCash(ThisObject,Cancel,CashDesk,Amount,AmountNational,AccumulationRecordType.Expense);
//		
//	If OperationType <> Enums.OperationTypesAccountsIncoming.Other Then
...
//		EndIf;
//	EndIf;
//	APAR.PostCashFlowTurnovers(Amount, AmountNational, CashDesk.Currency, ThisObject, AccumulationRecordType.Expense, Cancel);
//	
//EndProcedure
...
//	
//	If OperationType = Enums.OperationTypesAccountsOutgoing.Other Then
//		AttributesStructure = New Structure("Company, OperationType, CashDesk, Amount, AmountNational, ExchangeRate, SettlementCurrency, SettlementExchangeRate");
//	Else
//		AttributesStructure = New Structure("Company, OperationType, Partner, CashDesk, Amount, AmountNational, ExchangeRate, SettlementCurrency, SettlementExchangeRate, SettlementAmount");
//	EndIf;
...
//	
//	If  OperationType <> Enums.OperationTypesAccountsOutgoing.Other 
//		AND CashDesk.Currency = SettlementCurrency
//		AND Amount <> SettlementAmount Then
//		Alerts.AddAlert(Nstr("en=""Amount and settlement amount should be the same when cash desk currency and settlement's currency are the same."";pl='Kwota oraz kwota rozrachunków powinne być takie same o ile takie same są waluta kasy oraz waluta rozrachunków';ru='Сумма и сумма взаиморасчетов должны быть одинаковые, в случае, если валюта кассы и валюта взаиморасчетов совпадают.'"),Enums.AlertType.Error,Cancel,ThisObject);
//	Else
//		TmpSettlementAmount = Round(Amount*ExchangeRate/SettlementExchangeRate,2);
//		If OperationType <> Enums.OperationTypesAccountsOutgoing.Other 
//			AND ABS(SettlementAmount - TmpSettlementAmount)>0.05 Then
...
//	EndIf;
//	
//	If SettlementAmount - SettlementPrepaymentAmount<> SettlementDocuments.Total("AmountDr") - SettlementDocuments.Total("AmountCr") Then
...
//		AND (ManualExchangeRate OR (CashDesk.Currency = SettlementCurrency 
...
//	EndIf;
//	
//	TempExchangeRate = CommonAtServer.GetExchangeRate(CashDesk.Currency, Date);
...
//Procedure SetCurrencyCashDesk(CashDeskCurrency) Export
//	CurrencyCashDesk = CashDeskCurrency;
//EndProcedure
