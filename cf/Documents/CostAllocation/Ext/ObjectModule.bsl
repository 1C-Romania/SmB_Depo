...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	ElsIf TypeOf(FillingData) = Type("DocumentRef.CustomerOrder")
		...
		Raise NStr("en='Cannot enter Cost allocation based on the job order.';ru='Нельзя ввести Распределение затрат на основании заказ-наряда!'");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Inventory.Total("Quantity") <> InventoryDistribution.Total("Quantity") Then
		...
		MessageText = NStr("en='Quantity of inventory does not match those of the allocation.';ru='Количество запасов не соответствует количеству распределения!'");
	...
	If Costs.Total("Amount") <> CostAllocation.Total("Amount") Then
		...
		MessageText = NStr("en='Amount of expenses does not correspond to the allocation amount.';ru='Сумма расходов не соответствует сумме распределения!'");
	...
	If Constants.FunctionalOptionInventoryReservation.Get() Then
		...
		For Each StringInventoryDistribution IN InventoryDistribution Do
			...
			If StringInventoryDistribution.Reserve > StringInventoryDistribution.Quantity Then
				...
				MessageText = NStr("en='In row No. %Number% of the ""Inventory allocation"" tabular section, the number of items for write-off from reserve exceeds the total inventory quantity.';ru='В строке №%Номер% табл. части ""Распределение запасов"" количество позиций к списанию из резерва превышает общее количество запасов.'");
