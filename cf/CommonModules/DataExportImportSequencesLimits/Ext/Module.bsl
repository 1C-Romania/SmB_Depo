...
Procedure BeforeExportType(Container, Serializer, MetadataObject, Cancel) Export
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en='Metadata object %1 cannot be processed by handler DataExportImportSequencesLimits.BeforeObjectExport().';ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхГраницПоследовательностей.ПередВыгрузкойОбъекта()!'"),
