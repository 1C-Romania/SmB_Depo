	...
	For Each ScriptItem IN ScriptApplicationPermissions Do
		...
		If ScriptItem.Operation = OperationKinds.Creating Then
			...
			If ClusterAdministrationClientServer.SecurityProfileExists(ClusterAdministrationParameters, ScriptItem.Profile) Then
				...
				CommonUseClientServer.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Security profile %1 already exists in the server cluster. Settings in the security profile will be replaced...';ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...'"), ScriptItem.Profile));
		...
		ElsIf ScriptItem.Operation = OperationKinds.Delete Then
			...
			Else
				...
				CommonUseClientServer.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Security profile %1 does not exist in the server cluster. Security profile might have been deleted earlier...';ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...'"), ScriptItem.Profile));
...
//	Status - Structure - describing the status of the permissions settings assistant (see PermissionsToUseExternalResourcesRequestState))).
//
// Result of the peration is opening form
// "DataProcessor.PermissionSettingsForExternalResourcesUse.Form.EndPermissionsRequest" as for that notifications of closing description
// procedure AfterCompletingPermissionsQueryForExternalResourcesUsing or abort the preinstalled works assistant.
//
...
Procedure AfterCompletingPermissionsQueryForExternalResourcesUsing(Result, Status) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		ShowUserNotification(NStr("en='Set permissions';ru='Настройка разрешений'"),,
			NStr("en='Changes made to security profile settings in server cluster.';ru='Внесены изменения в настройки профилей безопасности в кластере серверов.'"));
