...
Procedure FullTextSearchUpdateIndex() Export
	...
	UpdateIndex(NStr("en='Update full-text search index';ru='Обновление индекса ППД'"), False, True);
...
Procedure FullTextSearchIndexMerge() Export
	...
	UpdateIndex(NStr("en='Merging full-text search index.';ru='Слияние индекса ППД'"), True);
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	Work.Presentation  = NStr("en='Full-text search index is out of date';ru='Индекс полнотекстового поиска устарел'");
	...
	Work.ToolTip      = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Last update was %1 ago';ru='Последнее обновление %1 назад'"), Interval);
	Work.Owner       = Section;
...
Procedure UpdateIndex(ProcedureRepresentation, AllowMerge = False, ByPortions = False)
	...
	LogRecord(Undefined, NStr("en='Start the ""%1"" procedure.';ru='Запуск процедуры ""%1"".'"), , ProcedureRepresentation);
	
	Try
		FullTextSearch.UpdateIndex(AllowMerge, ByPortions);
		LogRecord(Undefined, NStr("en='Procedure ""%1"" is completed successfully.';ru='Успешное завершение процедуры ""%1"".'"), , ProcedureRepresentation);
	Except
		LogRecord(Undefined, NStr("en='An error occurred when executing procedure ""%1"":';ru='Ошибка выполнения процедуры ""%1"":'"), ErrorInfo(), ProcedureRepresentation);
	EndTry;
