...
Procedure CreateUndividedPredefinedItems() Export
	...
	If CommonUseReUse.DataSeparationEnabled() AND CommonUseReUse.CanUseSeparatedData() Then
		...
		Raise NStr("en='Operation cannot be performed only in a session in which separator values are set';ru='Операция не может быть выполнена только в сеансе, в котором установлены значения разделителей'");
...
Procedure WhenGeneratingConfigurationManifest(AdvancedInformation) Export
	...
	If TransactionActive() Then
		Raise NStr("en='Operation cannot be completed when an external transaction is active.';ru='Операция не может быть выполнена при активной внешней транзакции!'");
	EndIf;
