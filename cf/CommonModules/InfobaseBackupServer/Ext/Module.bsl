...
Function CurrentBackupSetting() Export
	...
	If BackupSettings = Undefined Then
		Return NStr("en='To configure backup, contact administrator.';ru='Для настройки резервного копирования необходимо обратиться к администратору.'");
	EndIf;
	
	CurrentSetting = NStr("en='Backup is not configured. It can expose infobase to risks of data loss.';ru='Резервное копирование не настроено, информационная база подвергается риску потери данных.'");
	...
	If CommonUse.FileInfobase() Then
		...
		If BackupSettings.ExecuteAutomaticBackup Then
			...
			If BackupSettings.ExecutionVariant = "OnWorkCompletion" Then
				CurrentSetting = NStr("en='Backup is made regularly on closing the application.';ru='Резервное копирование выполняется регулярно при завершении работы.'");
			ElsIf BackupSettings.ExecutionVariant = "OnSchedule" Then // On schedule
				...
				If Not IsBlankString(Schedule) Then
					CurrentSetting = NStr("en='Regular backup on schedule: %1';ru='Резервное копирование выполняется регулярно по расписанию: %1'");
		...
		Else
			...
			If BackupSettings.BackupIsConfigured Then
				CurrentSetting = NStr("en='Backup is not running (organized by external applications).';ru='Резервное копирование не выполняется (организовано сторонними программами).'");
			EndIf;
	...
	Else
		...
		CurrentSetting = NStr("en='Backup is not running (organized by DBMS resources).';ru='Резервное копирование не выполняется (организовано средствами СУБД).'");
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	For Each Section IN Sections Do
		...
		If Not NotificationOnBackupConfigurationDisabled Then
			...
			Work.Presentation  = NStr("en='Configure backup';ru='Настроить резервное копирование'");
			Work.Important         = True;
		...
		If Not BackupNotificationDisabled Then
			...
			Work.Presentation  = NStr("en='Backup not completed';ru='Резервное копирование не выполнено'");
			Work.Important         = True;
