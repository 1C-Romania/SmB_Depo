...
Procedure ChoiceProcessingCertificate()
	...
	If Not ValueIsFilled(SelectedCertificate) Then
		Return;
	EndIf;
	If ElectronicDocumentsServiceCallServer.PasswordToCertificate(SelectedCertificate) Then
		UserPassword = Password;
		RememberPasswordDuringSession = Not Parameters.WriteToIB;
	Else
		Map = GetFromTempStorage(StorageAddress);
		CertificateAttributes = Map.Get(SelectedCertificate);
		UserPassword = ?(CertificateAttributes.PasswordReceived = True, CertificateAttributes.UserPassword, Undefined);
		RememberPasswordDuringSession = False;
		PasswordSaved = CertificateAttributes.RememberCertificatePassword;
	EndIf;
	Items.RememberPasswordDuringSession.Enabled = Not (PasswordSaved OR Parameters.WriteToIB);
	Items.UserPassword.Enabled = Not PasswordSaved OR Parameters.WriteToIB;
...
Procedure CommandDone(Command)
	...
	If Parameters.WriteToIB Then
		
		If UserPassword = "" Then
			
			AdditionalParameters = New Structure;			
			NotificationHandler = New NotifyDescription("SetBlankPasswordDescription", ThisObject, AdditionalParameters);
			
			QuestionText = NStr("en='Blank password set. Continue?';ru='Задан пустой пароль. Продолжить?'");
			
			ShowQueryBox(NotificationHandler, QuestionText, QuestionDialogMode.YesNo, 30, DialogReturnCode.Yes);
...
Procedure SelectedCertificateOpen(Item, StandardProcessing)
	...
	OpenForm("Catalog.DeleteDSCertificates.Form.ItemForm", FormParameters, ThisObject);
