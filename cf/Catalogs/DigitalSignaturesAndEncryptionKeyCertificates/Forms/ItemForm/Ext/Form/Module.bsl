...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	For Each KeyAndValue IN AttributesParameters Do
		...
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The ""%1"" field is not filled in.';ru='Поле %1 не заполнено.'"), Items[AttributeName].Title);
...
Procedure CheckCertificate(Command)
	...
	If Not ValueIsFilled(Object.Ref) Then
		ShowMessageBox(, NStr("en='Certificate has not been recorded yet.';ru='Сертификат еще не записан.'"));
		Return;
	EndIf;
...
Procedure CertificateRevoked(Command)
	...
	If Object.Revoked Then
		ShowMessageBox(, NStr("en='After writing, callback cannot be canceled.';ru='После записи отменить отзыв будет невозможно.'"));
	EndIf;
