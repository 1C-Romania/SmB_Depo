...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CancelGLAccountWithCustomerChange(Ref) Then
		Items.WithCustomer.ToolTip = NStr("en='Records are registered for this customer in the infobase. Cannot change the GL accounts for settlements with this customer.';ru='В базе есть движения по этому покупателю! Изменение счетов учета расчетов с покупателем запрещено!'");
		Items.WithCustomer.Enabled = False;
	EndIf;
		
	If CancelGLAccountWithVendorChange(Ref) Then
		Items.WithVendor.ToolTip = NStr("en='Records are registered for this supplier in the infobase. Cannot change the GL accounts for settlements with this supplier.';ru='В базе есть движения по этому поставщику! Изменение счетов учета расчетов с поставщиком запрещено!'");
