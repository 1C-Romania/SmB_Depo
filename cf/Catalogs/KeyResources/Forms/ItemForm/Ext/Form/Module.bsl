...
Procedure MarkSelectedAsNonWorkingYear()
	...
	If ThereAreSuitableDays
	   AND CurSchedule = Undefined Then
		ShowMessageBox(Undefined,NStr("en='Set the schedule for the selected days first.';ru='Установите вначале график для выбранных дней!'"));
	EndIf;
...
Procedure MarkSelectedAsNonWorkingDay()
	...
	If CurSchedule = Undefined Then
		ShowMessageBox(Undefined,NStr("en='Set the schedule for this day first.';ru='Установите вначале график для этого дня!'"));
		Return;
	EndIf;
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	If RadioButton = "Year" Then
		...
		DayNumber = SelectedArea.Left - 1;
		If MonthNumber < 1 OR MonthNumber > 12 OR DayNumber < 1 OR DayNumber > 31 OR DayNumber > NumberOfDaysInMonthAtClient(MonthNumber, Year(ScheduleDate)) Then
			ShowMessageBox(Undefined,NStr("en='Select the schedule setting day first.';ru='Вначале необходимо выделить день установки графика!'"));
			Return;
		EndIf;
