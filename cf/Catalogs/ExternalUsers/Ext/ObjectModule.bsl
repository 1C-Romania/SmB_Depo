...
Procedure BeforeWrite(Cancel)
	...
	If Not ValueIsFilled(AuthorizationObject) Then
		Raise NStr("en='Authorization object is not set for an external user.';ru='У внешнего пользователя не задан объект авторизации.'");
	Else
		ErrorText = "";
	...
	Else
		...
		If ValueIsFilled(OldAuthorizationObject)
		   AND OldAuthorizationObject <> AuthorizationObject Then
			...
			Raise NStr("en='Cannot change previously specified authorization object.';ru='Невозможно изменить ранее указанный объект авторизации.'");
		EndIf;
	EndIf;
