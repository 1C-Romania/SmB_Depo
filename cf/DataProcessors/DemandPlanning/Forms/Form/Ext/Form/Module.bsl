...
Procedure AddProductsAndServicesCharacteristic(ProductsAndServicesCurrent, StructureDetailing, StructureDetails, StartDate, EndDate)
	...
	If OnlyDeficit AND StructureDetailing.Deficit.IndicatorValue > 0 
		...
		OpeningBalance.ProductsAndServices = NStr("en='Opening property balance';ru='Начальные остатки по имуществу'");
		...
		Demand.ProductsAndServices = NStr("en='Token expiration date';ru='Срок годности токена'");
		...
		Else
			...
			MinInventory.ProductsAndServices = NStr("en='Minimum inventory';ru='Минимальный запас'");
			
			MaxInventory = ProductsAndServicesItems.Add();
			MaxInventory.ProductsAndServices = NStr("en='Maximum inventory';ru='Максимальный запас'");
...
Procedure GenerateOrdersAtServer()
	...
	For Each OrderParameters in TableOrders Do
		...
		CurrentOrder.Comment = NStr("en='Automatically generated by the ""Inventory demand calculation"" service.';ru='Сформирован автоматически сервисом ""Расчет потребностей в запасах"".'");
...
Procedure SetConditionalAppearance(BeginOfPeriod, EndOfPeriod)
	...
	FilterItem.LeftValue = New DataCompositionField("Inventory.ProductsAndServices");
	FilterItem.ComparisonType = DataCompositionComparisonType.Equal;
	FilterItem.RightValue = NStr("en='Token expiration date';ru='Срок годности токена'");
...
Procedure Reread(Command)
	...
	If ThisForm.Modified Then
		...
		QuestionStr = NStr("en='Data is changed. Reread?';ru='Данные изменены. Перечитать данные?'");
