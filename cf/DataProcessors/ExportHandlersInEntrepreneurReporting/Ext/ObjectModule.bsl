		...
		If Ref.Posted Then // For non-posted documents export a blank record set
			...
			If TypeOf(Ref) = Type("DocumentRef.CashReceipt") Then
				...
				If Ref.OperationKind = Enums.OperationKindsCashReceipt.FromCustomer Then
					OperationKindRow = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en='Cash receipt from customer ""%1""';ru='Поступление наличных денежных средств от покупателя ""%1""'"), TrimAll(Ref.Counterparty));
				...
				ElsIf Ref.OperationKind = Enums.OperationKindsCashReceipt.FromAdvanceHolder Then
					OperationKindRow = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en='Cash receipt from advance holder ""%1""';ru='Поступление наличных денежных средств от подотчетника ""%1""'"), TrimAll(Ref.AdvanceHolder));
			...
			ElsIf TypeOf(Ref) = Type("DocumentRef.CashPayment") Then
				...
				If Ref.OperationKind = Enums.OperationKindsCashPayment.Vendor Then
					OperationKindRow = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en='Cash expense for payment to supplier ""%1""';ru='Расход наличных денежных средств на оплату поставщику ""%1""'"), TrimAll(Ref.Counterparty));
				ElsIf Ref.OperationKind = Enums.OperationKindsCashPayment.ToCustomer Then
					OperationKindRow = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en='Cash expense for payment to customer ""%1""';ru='Расход наличных денежных средств на оплату покупателю ""%1""'"), TrimAll(Ref.Counterparty));
				ElsIf Ref.OperationKind = Enums.OperationKindsCashPayment.ToAdvanceHolder Then
					OperationKindRow = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en='Cash dispense to advance holder ""%1""';ru='Выдача наличных денежных средств подотчетнику ""%1""'"), TrimAll(Ref.AdvanceHolder));
			...
			ElsIf TypeOf(Ref) = Type("DocumentRef.PaymentExpense") Then
				...
				ElsIf Ref.OperationKind = Enums.OperationKindsPaymentExpense.ToAdvanceHolder Then
					OperationKindRow = StringFunctionsClientServer.PlaceParametersIntoString(NStr("en='Non-cash payment to advance holder ""%1""';ru='Выдача безналичных денежных средств подотчетнику ""%1""'"), TrimAll(Ref.AdvanceHolder));
