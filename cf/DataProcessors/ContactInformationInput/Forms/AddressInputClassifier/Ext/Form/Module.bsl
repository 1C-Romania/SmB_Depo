...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	SetItemSelectionList(Items.UnitType,      	  Items.Unit,       ContactInformationInternal.DataOptionsUnit());
	...
	If IsBlankString(Unit) And IsBlankString(UnitType) Then
		UnitType = ContactInformationClientServer.FirstOrEmpty(Items.UnitType);
	EndIf;
...
Function ChoiceResult(Context, ReturnValueList = False)
	...
	//conversion to US address format
	Address = XDTOInformation.Content;
	AddressUS = Address.Content;
	If TypeOf(AddressUS) = Type("XDTODataObject") Then
		Address.AddressLine1 = ContactInformationClientServer.FullDescr(Context.Building + ?(IsBlankString(Context.Unit), "", "-" + Context.Unit), "", TrimAll(AddressUS.Street), "", TrimAll(AddressUS.Settlement));
		Address.AddressLine2 = "";
		Address.City = AddressUS.City;
		Address.State = AddressUS.Region;
		Address.PostalCode = Context.PostalCode;
		XDTOInformation.Presentation = ContactInformationInternal.AddressPresentation(Address, Context.ContactInformationKind);	
	EndIf;
...
Procedure ContactInformationAttibuteValues(Context, InformationToEdit)
	...
	If ContactInformationInternal.IsDomesticAddress(AddressData) Then
		Context.AllowAddressInputInFreeFormat = Not IsBlankString(AddressData.Content.Address_by_document);
		...
		If Context.AllowAddressInputInFreeFormat Then
			If AddressPresentationsIdentical(CalculatedPresentation, AddressData.Content.Address_by_document, True) 
				And AddressPresentationsIdentical(InformationToEdit.Presentation, AddressData.Content.Address_by_document, True) 
			Then
				Context.AllowAddressInputInFreeFormat = False;
				AddressData.Content.Address_by_document      = "";
			EndIf;
		EndIf;
		
		If Context.AllowAddressInputInFreeFormat Then
			Context.AddressPresentation = AddressData.Content.Address_by_document;
		EndIf;
	Else
	...
	// Kind = 1 - flag used to specify a building. Kind = 2 - unit
	...
	BuildingString = DataTable.Find(2, "Kind");
	If BuildingString<>Undefined Then
		Context.UnitType = BuildingString.Type;
		Context.Unit     = BuildingString.Value;
		DataTable.Delete(BuildingString);
	EndIf;
...
Function ContactInformationByAttributeValues(Context)
	...
	If Not IsBlankString(Context.Building) Then
		BuildingRow = BuildingTable.Insert(0);
		BuildingRow.Type = Context.BuildingType;
		BuildingRow.Value = Context.Building;
	EndIf;
	
	If Not IsBlankString(Context.Unit) Then
		BuildingRow = BuildingTable.Insert(0);
		BuildingRow.Type  = Context.UnitType;
		BuildingRow.Value = Context.Unit;
	EndIf;
	
	ContactInformationInternal.BuildingAddresses(AddressUS, New Structure("Buildings", BuildingTable));
	...
	If Context.AllowAddressInputInFreeFormat And Context.AddressPresentationChanged Then
		...
		Else
			...
			AddressUS.Address_by_document = EnteredPresentation;
		EndIf;
	Else
		AddressUS.Unset("Address_by_document");
		Result.Presentation = ExpectedPresentation;
...
Function PathToFormDataByXPath(XPath) 
	...
	// Unit options
	For Each ListItem In Items.UnitType.ChoiceList Do
		If XPath = ContactInformationClientServerCached.AdditionalAddressingObjectNumberXPath(ListItem.Value) Then
			Return "Unit";
		EndIf;
	EndDo;
