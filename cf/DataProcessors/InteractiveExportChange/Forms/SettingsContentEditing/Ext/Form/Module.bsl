...
Procedure SettingsOptionsBeforeDeleting(Item, Cancel)
	...
	QuestionText   = NStr("en='Remove setting ""%1""?';ru='Удалить настройку ""%1""?'");
...
Procedure SaveSetting(Command)
	...
	If IsBlankString(ViewCurrentSettings) Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Name for the current setting is not entered.';ru='Не заполнено имя для текущей настройки.'"), , "ViewCurrentSettings");
		Return;
	EndIf;
		
	If SettingVariants.FindByValue(ViewCurrentSettings)<>Undefined Then
		HeaderText = NStr("en='Confirmation';ru='Подтверждение'");
		QuestionText   = NStr("en='Rewrite existing setting ""%1""?';ru='Перезаписать существующую настройку ""%1""?'");
