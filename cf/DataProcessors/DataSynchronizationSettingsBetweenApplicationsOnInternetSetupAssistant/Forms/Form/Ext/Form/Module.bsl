...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Title = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Setting of data synchronization with ""%1""';ru='Настройка синхронизации данных с ""%1""'"), CorrespondentDescription);
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	WarningText = NStr("en='Cancel data synchronization setting?';ru='Отменить настройку синхронизации данных?'");
...
Procedure ShowMessageAboutError(Cancel)
	...
	CommonUseClientServer.MessageToUser(NStr("en='Cannot execute the operation.';ru='Не удалось выполнить операцию.'"),,,, Cancel);
...
Function Attachable_Begin_OnGoingNext(Cancel)
	...
	If NodeFiltersSettingsAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en='Configure required settings.';ru='Необходимо задать обязательные настройки.'"),,, "DataExportSettingsDescription", Cancel);
...
Procedure DataAnalysisExpectation_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (step 1)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)'"));
...
Procedure RunAutomaticDataMapping(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (automatic data mapping)';ru='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)'"));
...
Procedure GetStatsComparison(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (mapping statistics receipt)';ru='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)'"));
...
Procedure DataSynchronizationExpectation_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (catalog synchronization)';ru='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)'"));
...
Procedure ImportParametersAccountingCorrespondent(Cancel, ErrorInfo = "", CorrespondentErrorMessage = "")
	...
	Try
		...
		If IsBlankString(CorrespondentErrorMessage) Then
			CorrespondentErrorMessage = NStr("en='Accounting parameters are not specified in the ""%1"" application.';ru='Не заданы параметры учета в приложении ""%1"".'");
		...
		If IsBlankString(ErrorInfo) Then
			ErrorInfo = NStr("en='Accounting parameters are not specified in this application.';ru='Не заданы параметры учета в этом приложении.'");
		EndIf;
...
Function Attachable_DataReceivingRules_OnGoingNext(Cancel)
	...
	If NodeDefaultValuesAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en='Configure required settings.';ru='Необходимо задать обязательные настройки.'"),,, "ValuesDescriptionFullByDefault", Cancel);
	...
	If CorrespondentInfobaseNodeDefaultValuesAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en='Specify required settings for the second application.';ru='Необходимо задать обязательные настройки для второго приложения.'")
...
Procedure SettingsSaveExpected_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (step 2)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)'"));
