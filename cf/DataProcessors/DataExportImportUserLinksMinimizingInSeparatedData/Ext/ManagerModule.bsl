...
//          should use SerializerXDTO that is passed to the procedure BeforeObjectExport()
...
//        Container - DataProcessorObject.DataExportImportContainerManager - container manager that is used in the process of data export. See more
//          the review to the application data processor interface DataExportImportContainerManager, 
//        Serializer - XDTOSerializer, initiated with support of references abstracts execution. If a random exporting handler requires additional data export //          - You should use SerializerXDTO that is passed to
...
Procedure WhenDataImportHandlersRegistration(HandlersTable) Export
	...
	HandlerObject = DataProcessors.DataExportImportUserLinksMinimizingInSeparatedData.Create();
...
//        Artifacts - Array(XDTOObject) - additional data that is logically inextricably associated with the data object, but not being a part of it. 
//          Created in exported procedures BeforeObjectExport() of data export handlers (see a comment to procedure OnDataExportHandlersRegistration(). Each artifact must be the XDTOobject for which type, as the base type, abstract XDTOtype is used {http://www.1c.ru/1cFresh/Data/Dump/1.0.2.1}Artefact. It is
...
Procedure WhenDataExportHandlersRegistration(HandlersTable) Export
	...
	ListOfRegisters = UsersServiceSaaSSTLReUse.RecordSetListWithReferencesToUsers();
	For Each ItemOfList IN ListOfRegisters Do
		
		NewHandler = HandlersTable.Add();
		NewHandler.MetadataObject = ItemOfList.Key;
		NewHandler.Handler = HandlerObject;
