...
Procedure BeforeObjectExport(Container, ObjectExportManager, Serializer, Object, Artifacts, Cancel) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Metadata object %1 cannot be processed by handler ServiceUsersInBTCSaaS.BeforeObjectExport().';ru='Объект метаданных %1 не может быть обработан обработчиком ПользователиСлужебныйВМоделиСервисаБТС.ПередВыгрузкойОбъекта()!'"),
...
Procedure BeforeMatchRefs(Container, MetadataObject, SourceRefsTable, StandardProcessing, Cancel) Export
	...
	Else
		...
		Raise NStr("en='Data type is incorrect';ru='Тип данных указан неверно'");
...
Procedure BeforeObjectImport(Container, Object, Artifacts, Cancel) Export
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.PlaceParametersIntoString(
			NStr("en='Metadata object %1 cannot be processed by handler ServiceUsersInBTCSaaS.BeforeObjectImport().';ru='Объект метаданных %1 не может быть обработан обработчиком ПользователиСлужебныйВМоделиСервисаБТС.ПередЗагрузкойОбъекта()!'"),
			Object.Metadata().FullName());
