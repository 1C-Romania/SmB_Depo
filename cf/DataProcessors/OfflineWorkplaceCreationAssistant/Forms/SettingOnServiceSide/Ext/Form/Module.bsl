...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ItemTitle = NStr("en='To work offline, 1C:Enterprise 8.3 platform of version [PlatformVersion] should be installed on your computer';ru='Для автономной работы на Вашем компьютере должна быть установлена платформа ""1С:Предприятие 8.3"" версии [ВерсияПлатформы]'");
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	WarningText = NStr("en='Cancel the offline work place creation?';ru='Отменить создание автономного рабочего места?'");
...
Procedure SaveInstructionAs(Command)
	...
	GetFile(AddressInTemporaryStorage, NStr("en='Offline work place setup instruction.html';ru='Инструкция по настройке автономного рабочего места.html'"));
...
Procedure CreateInitialImageOfOfflineWorkspaceOnServer(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
						...
						ContextAssistant,
						NStr("en='Creating initial image of offline work place';ru='Создание начального образа автономного рабочего места'"),
						True);
...
Function AddressOfApplicationOnWeb()
	...
	If Not ConnectionParameters.Property("ws") Then
		Raise NStr("en='Offline work place can be created only in the web client mode.';ru='Создать автономное рабочее место можно только в режиме веб-клиента.'");
	EndIf;
...
Function Attachable_SettingExportings_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.OfflineWorkplaceDescription) Then
		...
		NString = NStr("en='Offline work place name is not specified.';ru='Не задано наименование автономного рабочего места.'");
