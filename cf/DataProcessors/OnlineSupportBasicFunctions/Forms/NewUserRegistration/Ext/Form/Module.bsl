...
Function FieldsAreFilledCorrectly()
	...
	If IsBlankString(Login) Then
		MessageToUser(
			NStr("en='Login is not filled in.';ru='Поле ""Логин"" не заполнено.'"),
			"Login",
			Cancel);
	EndIf;
	...
	If IsBlankString(Password) Then
		MessageToUser(
			NStr("en='Password is not filled in.';ru='Поле ""Пароль"" не заполнено.'"),
			"Password",
			Cancel);
	ElsIf Password <> PasswordConfirmation Then
		MessageToUser(
			NStr("en='Password and its confirmation do not match.';ru='Не совпадают пароль и его подтверждение.'"),
	...
	If IsBlankString(Email) Then
		MessageToUser(
			NStr("en='Email is not filled in.';ru='Поле ""E-mail"" не заполнено.'"),
			"Email",
			Cancel);
	EndIf;
...
Function MessageParametersToTechicalSupport()
	...
	Result.Insert("Subject"  , NStr("en='Online support. Register new user.';ru='Интернет-поддержка. Регистрация нового пользователя.'"));
	Result.Insert("FromWhom", Email);
	
	MessageText = NStr("en='Dear Sir or Madam, I cannot register a new user to connect online support. Please help me to deal with the issue. Login: %1 Email: %2 Last name: %3 Name: %4 Patronymic: %5 City: %6 Phone: %7 Place of employment: %8. %TechnicalParameters% ----------------------------------------------- Sincerely, .';ru='Здравствуйте! У меня не получается зарегистрировать нового пользователя для подключения Интернет-поддержки. Прошу помочь разобраться с проблемой. Логин: %1 E-mail: %2 Фамилия: %3 Имя: %4 Отчество: %5 Город: %6 Телефон: %7 Место работы: %8. %ТехническиеПараметры% ----------------------------------------------- С уважением, .'");
