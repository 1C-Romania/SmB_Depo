...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(OfflineWorkplace) Then
		Raise NStr("en='An offline work place is not specified.';ru='Не задано автономное рабочее место.'");
	EndIf;
...
Procedure LongOperationIdleHandler()
	...
	Except
		...
		LongOperation = False;
		SkipBack();
		ShowMessageBox(,NStr("en='Errors occurred during the operation.';ru='В процессе работы возникли ошибки.'"));
...
Procedure DeleteOfflineWorkplace(Cancel)
	...
	Result = LongActions.ExecuteInBackground(
					...
					NStr("en='Deletion of offline work place';ru='Удаление автономного рабочего места'"));
