		...
		If ReportVariantsCommandsVisible // Commands are not enabled by the form opening parameters.
			...
			If ValueIsFilled(PredefinedVariant) Then
				...
				If Disabled.Find(PredefinedVariant) <> Undefined Then // Option is disabled.
					Text = NStr("en='Context variant ""%1"" of the ""%2"" report is opened without context.';ru='Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.'");
	...
	If TypeOf(ParametersForm.Filter) = Type("Structure") Then
		...
		For Each KeyAndValue In ParametersForm.Filter Do
			...
			If TypeOf(AvailableDCField) = Type("DataCompositionFilterAvailableField") Then
				...
				Continue;
			EndIf;
			ErrorText = NStr("en='Cannot set fixed filter ""%1"".';ru='Не удалось установить фиксированный отбор ""%1"".'");
	...
	For Each DCUsersSetting In DCUserSettings.Items Do
		...
		If Type = "SettingsParameterValue"
			...
			ElsIf Value.StartDate > Value.EndDate Then
				ErrorText = NStr("en='Period end should be later than its start';ru='Конец периода должен быть больше начала'");
...
Procedure Attachable_LoadReportVariant(Command)
	...
	If Found.Count() = 0 Then
		ShowMessageBox(, NStr("en='Report variant was not found.';ru='Вариант отчета не найден.'"));
		Return;
	EndIf;
	FormVariant = Found[0];
...
Procedure BackgroundJobCheckAtClient()
	If BackGroundJobFinished() Then
		ShowUserNotification(NStr("en='Report is generated';ru='Отчет сформирован'"), , Title);
...
Procedure VisibleEnabledCorrectness(Changes = "")
	...
	EndIf;
	If EncryptingMode Then
		Title = Title + " (" + NStr("en='Explanation';ru='Расшифровка'") + ")";
	EndIf;
...
Function BackGroundJobStart()
	...
	Try
		BackgroundJobResult = LongActions.ExecuteInBackground(
			...
			NStr("en='Report variants: Report generation';ru='Варианты отчетов: Формирование отчета'"));
	Except
		ErrorInfo = ErrorInfo();
	EndTry;
	AdditProperties.Delete("VariantKey");
...
Procedure LoadVariant(VariantKey)
	...
	StatePresentation.Text     = NStr("en='Another report variant is selected. Click ""Generate"" to receive the report.';ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.'");
EndProcedure
...
Procedure FormationErrorsShow(ErrorInfo)
	If TypeOf(ErrorInfo) = Type("ErrorInfo") Then
		...
		DetailErrorDescription = NStr("en='Generation error:';ru='Ошибка при формировании:'") + Chars.LF + DetailErrorDescription(ErrorInfo);
...
Procedure AfterChangingKeyStates(FillingParameters)
	If FillingParameters.Event.Name <> "AfterGenerating" Then
		...
		ElsIf FillingParameters.VariantModified
			...
			StatePresentation = Items.ReportSpreadsheetDocument.StatePresentation;
			StatePresentation.Visible = True;
			StatePresentation.Text     = NStr("en='Settings were changed. Click ""Generate"" to get the report.';ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.'");
