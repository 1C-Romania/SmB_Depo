...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else // no order
		Order = Documents.CustomerOrder.EmptyRef();
		NewRow = OrdersList.Add();
		NewRow.Order = Documents.CustomerOrder.EmptyRef();
		NewRow.Total = Parameters.DocumentAmount;
		NewRow.TotalCalc = SmallBusinessServer.RecalculateFromCurrencyToCurrency(
			Parameters.DocumentAmount,
			?(SettlementsCurrency = DocumentCurrency, ExchangeRate, 1),
...
EndProcedure // PrepaymentAccountsAmountOnChange()
...
EndProcedure // PrepaymentRateOnChange()
...
EndProcedure // PrepaymentMultiplicityOnChange()
...
Procedure FillAdvances()
	...
	|		ISNULL(AccountsReceivableBalances.AmountBalance, 0) AS AccountingAmount,
	|		ISNULL(AccountsReceivableBalances.AmountCurBalance, 0) AS SettlementsAmount,
	|		ISNULL(AccountsReceivableBalances.AmountBalance, 0) * SettlementsCurrencyCurrencyRates.ExchangeRate * DocumentCurrencyCurrencyRates.Multiplicity / (DocumentCurrencyCurrencyRates.ExchangeRate * SettlementsCurrencyCurrencyRates.Multiplicity) AS PaymentAmount
	|	FROM
	|		TemporaryTableAccountsReceivableBalances AS AccountsReceivableBalances
