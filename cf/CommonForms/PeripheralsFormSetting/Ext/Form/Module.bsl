...
Procedure OpenExchangeRulesWithPeripherals(Command)
	...
	If Modified Then
		...
		MessageText = NStr("en='Data is not written yet. You can go to the settings only after the data is written. Write?';ru='Данные еще не записаны! Переход к настройкам возможен только после записи данных! Записать?'");
		Response = Undefined;
