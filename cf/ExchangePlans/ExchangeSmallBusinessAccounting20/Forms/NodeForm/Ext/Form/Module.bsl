...
Procedure OpenFilterFormByCompanies(Command)
	...
	OpenForm("ExchangePlan.ExchangeSmallBusinessAccounting20.Form.ChoiceFormCompanies",
		FormParameters,
		ThisForm);
...
Procedure OpenFormFilterByDocumentTypes(Command)
	...
	OpenForm("ExchangePlan.ExchangeSmallBusinessAccounting20.Form.DocumentKindsChoiceForm",
		FormParameters,
		ThisForm);
...
Procedure RefreshDescriptionCommandsForms()
	...
	If Object.Companies.Count() > 0 Then
		...
		NewTitleCompanies = ExchangePlans.ExchangeSmallBusinessAccounting20.ShortPresentationOfCollectionsOfValues(SelectedCompanies);
	...
	If Object.DocumentKinds.Count() > 0 Then
		...
		NewTitleOfDocuments = ExchangePlans.ExchangeSmallBusinessAccounting20.ShortPresentationOfCollectionsOfValues(SelectedDocumentKinds);
...
Procedure OnChangeOfDataSynchronization()
	...
	If SynchronizationMode = "AutomaticSynchronization" Then
		...
		Items.ImportDocumentsFromAccountingBySubsidiaryCompanyGroup.Enabled = False;
		
	Else
		
		Items.DataSendConditionsGroup.Enabled = False;
		Items.ImportDocumentsFromAccountingBySubsidiaryCompanyGroup.Enabled = True;
