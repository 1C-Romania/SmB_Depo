...
Function CommonNodeData(CorrespondentVersion, FormName) Export
	...
	Return "DocumentsExportStartDate, Companies, ExportModeOnDemand, ManualExchange";
...
Function DetailedInformationAboutExchange(SettingID) Export
	...
	Else
		PathToExchangeInformation = "ExchangePlan.ExchangeSmallBusinessAccounting30.Form.ExchangeDetailedInformationForm";
	EndIf;
...
Procedure InitializeExportModeOnDemand() Export
	...
	Query = New Query("SELECT
	                      |	PSU.Ref
	                      |FROM
	                      |	ExchangePlan.ExchangeSmallBusinessAccounting30 AS PSU
	                      |WHERE
	                      |	PSU.Ref <> &ThisNode
	                      |	AND CASE
	...
	Query.SetParameter("ThisNode", ExchangePlans.ExchangeSmallBusinessAccounting30.ThisNode());	
