...
Function InformationExchangeAbsoluteDirectory(Val Correspondent) Export
	...
	If QueryResult.IsEmpty() Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Connection settings for the ""%1"" correspondent are not specified.';ru='Не заданы настройки подключения для корреспондента ""%1"".'"), String(Correspondent));
	EndIf;
...
Function TransportSettingsWS(Val Correspondent) Export
	...
	If QueryResult.IsEmpty() Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Web service connection settings are not specified for the correspondent %1.';ru='Не заданы настройки подключения веб-сервиса для корреспондента %1.'"),
