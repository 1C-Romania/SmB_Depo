	...
	Else
		...
		If Context.AllowDataChangingToProhibitionDate Then
			...
			Else
				...
				EndIf;
				LabelText = Chars.LF
				             + NStr("en='For %2 data might change from %3 to %4';ru='По %2 возможно изменение данных с %3 по %4'")
				             + Chars.LF
				             + NStr("en='After %2 it will be prohibited to change %4 data';ru='После %2 будет запрещено изменение данных по %4'")
				             + Chars.LF;
			EndIf;
		Else
			Context.Items.PropertyPermissionDaysCountChanges.CurrentPage =
