...
Процедура ВыполнитьВыгрузку(Знач ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = Ложь) Экспорт
	...
	Если СоставПолнойВыгрузки.Количество() = 0
		...
		СообщитьПользователю(Нстр("ru='Не задан состав выгрузки';en='Не задан состав выгрузки'"));
		Возврат;
	...
	Иначе
		...
		Если ТолькоПроверкаНедопустимыхСимволов Тогда
			...
			ШаблонНачалоПроверки = Нстр("ru='Начало проверки: %Дата';en='Начало проверки: %Дата'");
		...
		Иначе
			...
			ШаблонНачалоВыгрузки = Нстр("ru='Начало выгрузки: %Дата';en='Начало выгрузки: %Дата'");
	...
	Если ТолькоПроверкаНедопустимыхСимволов Тогда
		...
		ШаблонПроверено = Нстр("ru='Проверено объектов: %Проверено';en='Проверено объектов: %Проверено'");
		СообщениеПроверено = СтрЗаменить(ШаблонПроверено, "%Проверено", ВсегоОбработанныхЗаписей());
		СообщитьПользователю(СообщениеПроверено);
		
		ШаблонОкончание = Нстр("ru='Окончание проверки: %Дата';en='Окончание проверки: %Дата'");
		СообщениеОкончание = СтрЗаменить(ШаблонОкончание, "%Дата", ТекущаяДатаСеанса());
		СообщитьПользователю(СообщениеОкончание);
		
	Иначе
		
		ШаблонВыгружено = Нстр("ru='Выгружено объектов: %Выгружено';en='Выгружено объектов: %Выгружено'");
		СообщениеВыгружено = СтрЗаменить(ШаблонВыгружено, "%Выгружено", ВсегоОбработанныхЗаписей());
		СообщитьПользователю(СообщениеВыгружено);
		
		ШаблонОкончание = Нстр("ru='Окончание выгрузки: %Дата';en='Окончание выгрузки: %Дата'");
		СообщениеОкончание = СтрЗаменить(ШаблонОкончание, "%Дата", ТекущаяДатаСеанса());
		СообщитьПользователю(СообщениеОкончание);
		
		СообщитьПользователю(Нстр("ru='Выгрузка данных успешно завершена';en='Выгрузка данных успешно завершена'"));
...
Процедура ВыполнитьЗагрузку(Знач ИмяФайла) Экспорт
	...
	ШаблонСообщения = Нстр("ru='Начало загрузки: %Дата';en='Начало загрузки: %Дата'");
	...
	Пока Сериализатор.ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Исключение
			...
			Иначе
				...
				Попытка
					ТекстСообщения = Нстр("ru='При загрузке объекта %1(%2) возникла ошибка: %3';
				...
				Исключение
					ТекстСообщения = Нстр("ru='При загрузке данных возникла ошибка: %1';
	...
	ШаблонЗагружено = Нстр("ru='Загружено объектов: %Количество';en='Загружено объектов: %Количество'");
	СообщениеЗагружено = СтрЗаменить(ШаблонЗагружено, "%Количество", Загружено);
	
	ШаблонОкончание = Нстр("ru='Окончание загрузки: %Дата';en='Окончание загрузки: %Дата'");
	СообщениеОкончание = СтрЗаменить(ШаблонОкончание, "%Дата", ТекущаяДатаСеанса());
	
	СообщитьПользователю(СообщениеЗагружено);
	СообщитьПользователю(СообщениеОкончание);
	СообщитьПользователю(Нстр("ru='Загрузка данных успешно завершена';en='Загрузка данных успешно завершена'"));
	
КонецПроцедуры
// Процедура ВыполнитьЗагрузку(Знач ИмяФайла) 
...
Функция СформироватьТаблицуОшибок(ОбъектыВыгруженныеСОшибками)
	...
	Если ОбъектыВыгруженныеСОшибками.Количество() = 0 Тогда
		СообщитьПользователю(Нстр("ru='Проверка объектов на наличие недопустимых символов завершена. Ошибок не обнаружено.';
								  |en='Проверка объектов на наличие недопустимых символов завершена. Ошибок не обнаружено.'"));
	Иначе
		...
		Для Каждого СтрокаТаблицыВыгрузки Из СоставПолнойВыгрузки Цикл
			...
			Если СтрокаДереваМетаданных.ЭлементОписания.Менеджер = Неопределено Тогда
				ВызватьИсключение(Нстр("ru='Выгрузка данных. Внутренняя ошибка';
									   |en='Data unload - internal errror'"));
			КонецЕсли;
...
Процедура ОбработкаРезультатаЗапроса(РезультатЗапроса, ЗаписьXML, ЭтоЗапросПоОбъекту = Ложь, 
	...
	Пока ВыборкаИзРезультатовЗапроса.Следующий() Цикл
		...
		Если ЭтоЗапросПоОбъекту Тогда
			...
			Ссылка = ВыборкаИзРезультатовЗапроса.ref;
		...
		Если ЭтоЗапросПоОбъекту Тогда
			...
			Попытка
				...
				Если ЭтоМетаданныеСПредопределеннымиЭлементами(МетаданныеОбъекта) И Объект.Предопределенный Тогда
					...
					НоваяСтрока.TableName = МетаданныеОбъекта.ПолноеИмя();
					НоваяСтрока.Ref = XMLСтрока(Ссылка);
					НоваяСтрока.PredefinedDataName = Объект.ИмяПредопределенныхДанных;
				КонецЕсли;
			...
			Исключение
				...
				Иначе
					...
					ИтоговаяСтрокаСообщения = Нстр("ru='При выгрузке объекта %1(%2) возникла ошибка: %3';
												   |en='При выгрузке объекта %1(%2) возникла ошибка: %3'");
...
Процедура ЗаписьКонстанты(ЗаписьXML, МД_Константа, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов)
	...
	Исключение
		...
		Иначе
			...
			ИтоговаяСтрокаСообщения = Нстр("ru='При выгрузке константы %1 возникла ошибка: %2';
										   |en='При выгрузке константы %1 возникла ошибка: %2'");
...
/////////////////////////////////////////////////////////////////////////////////////
// Процедура рекурсивно обрабатывает строку дерева метаданных, 
// образуя списки полной и вспомогательной выгрузки
//
// Параметры
...
/////////////////////////////////////////////////////////////////////////////////////
// Процедура заполняет строку дерева метаданных, попутно заполняя соответствие
// ссылочных типов объектам метаданных
//
// Параметры
...
/////////////////////////////////////////////////////////////////////////////////////
// Процедура удаляет из дерева метаданных строки, соответствующие метаданным, 
// заведомо не попадающим в выгрузку
//
// Параметры
...
/////////////////////////////////////////////////////////////////////////////////////
// Процедура проставляет признак Выгрузка строке дерева метаданных 
// на основании этого признака подчиненных строк,
...
/////////////////////////////////////////////////////////////////////////////////////
// Процедура обрабатывает состояние признака Выгрузка, 
// проставляя признаки Выгрузка и ВыгружатьПриНеобходимости
...
// соответствующего строке дерева
//
// Параметры
//   ЭлементДЗ - строка дерева метаданных
...
/////////////////////////////////////////////////////////////////////////////////////
// Процедура определяет, на какие объект может ссылаться запись, 
// соответствующая объекту метаданных, отображаемому
	...
	Для Каждого СсылочныйТип Из ВсеТипы Цикл
		...
		Если ТипИОбъект = Неопределено Тогда
			...
			ТекстИсключения = Нстр("ru='Внутренняя ошибка. Неполное заполнение структуры ссылочных типов %1';
								   |en='Внутренняя ошибка. Неполное заполнение структуры ссылочных типов %1'");
		...
		Если СтрокаДЗ = Неопределено Тогда 
			...
			ТекстИсключения = Нстр("ru='Внутренняя ошибка. Неполное заполнение дерева метаданных. Отсутствует объект, образующий тип %1';
								   |en='Внутренняя ошибка. Неполное заполнение дерева метаданных. Отсутствует объект, образующий тип %1'");
			ТекстИсключения = ПодставитьПараметрыВСтроку(ТекстИсключения, СсылочныйТип);
			ВызватьИсключение(ТекстИсключения);
...
Процедура ИнициализироватьТаблицуПредопределенных()
	...
	ТаблицаПредопределенных.Колонки.Добавить("TableName");
	ТаблицаПредопределенных.Колонки.Добавить("Ref");
	ТаблицаПредопределенных.Колонки.Добавить("PredefinedDataName");
...
Процедура ВыгрузитьТаблицуПредопределенных(ЗаписьXML)
	...
	Если ТаблицаПредопределенных.Количество() > 0 Тогда
		...
		ТаблицаПредопределенных.Сортировать("TableName");
		
		ИмяПредыдущейТаблицы = "";
		
		Для Каждого Элемент Из ТаблицаПредопределенных Цикл
			
			Если ИмяПредыдущейТаблицы <> Элемент.TableName Тогда
				Если Не ПустаяСтрока(ИмяПредыдущейТаблицы) Тогда
					ЗаписьXML.ЗаписатьКонецЭлемента();
				КонецЕсли;
				ЗаписьXML.ЗаписатьНачалоЭлемента(Элемент.TableName);
			КонецЕсли;
			
			ЗаписьXML.ЗаписатьНачалоЭлемента("item");
			ЗаписьXML.ЗаписатьАтрибут("Ref", Элемент.Ref);
			ЗаписьXML.ЗаписатьАтрибут("PredefinedDataName", Элемент.PredefinedDataName);
			ЗаписьXML.ЗаписатьКонецЭлемента();
			
			ИмяПредыдущейТаблицы = Элемент.TableName;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если ЧтениеXML.ЛокальноеИмя <> "item" Тогда
				...
				ВременнаяСтрока.TableName = ЧтениеXML.ЛокальноеИмя;
				
				ТекстЗапроса = 
				"ВЫБРАТЬ
				|	Таблица.Ref КАК Ссылка
				|ИЗ
				|	" + ВременнаяСтрока.TableName + " КАК Таблица
				|ГДЕ
				|	Таблица.PredefinedDataName = &ИмяПредопределенныхДанных";
			...
			Иначе
				...
				Запрос.УстановитьПараметр("ИмяПредопределенныхДанных", ВременнаяСтрока.PredefinedDataName);
				...
				Если Не РезультатЗапроса.Пустой() Тогда
					...
					Если Выборка.Количество() = 1 Тогда
						...
						СсылкаВФайле = ВременнаяСтрока.Ref;
					...
					Иначе
						...
						ТекстИсключения = СтрЗаменить(ТекстИсключения, "%1", ВременнаяСтрока.PredefinedDataName);
						ТекстИсключения = СтрЗаменить(ТекстИсключения, "%2", ВременнаяСтрока.TableName);
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	Если ВРег(КлассОМ) = "CATALOG" Тогда
		Менеджер = Справочники;
	//ИначеЕсли ВРег(КлассОМ) = "ПЛАНВИДОВХАРАКТЕРИСТИК" Тогда
	ИначеЕсли ВРег(КлассОМ) = "CHARTOFCHARACTERISTICTYPES" Тогда
		Менеджер = ПланыВидовХарактеристик;
	//ИначеЕсли ВРег(КлассОМ) = "ПЛАНСЧЕТОВ" Тогда
	ИначеЕсли ВРег(КлассОМ) = "CHARTOFACCOUNTS" Тогда
		Менеджер = ПланыСчетов;
	//ИначеЕсли ВРег(КлассОМ) = "ПЛАНВИДОВРАСЧЕТА" Тогда
	ИначеЕсли ВРег(КлассОМ) = "CHARTOFCALCULATIONTYPES" Тогда
