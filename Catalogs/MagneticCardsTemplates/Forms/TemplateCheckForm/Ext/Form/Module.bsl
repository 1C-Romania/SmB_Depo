	   ...
	   AND IsInputAvailable() Then
		If EventName = "TracksData" Then
			...
			If Not EquipmentManagerClient.CodeCorrespondsToMCTemplate(TracksData, PatternData) Then
				CommonUseClientServer.MessageToUser(NStr("en='Card does not matches with template!'"));
				Return;
			EndIf;
			...
			If Parameter[1][3] = Undefined
				...
				CommonUseClientServer.MessageToUser(NStr("en='It was not succeeded to distinguish any field. Perhaps, the template fields have been configured incorrectly.'"));
			Else
				...
				If TemplateFound = Undefined Then
					CommonUseClientServer.MessageToUser(NStr("en='Code does not match this template. Perhaps, template has been configured incorrectly.'"));
				Else
					MessageText = NStr("en='Card matches to template and contains this fields:'")+Chars.LF+Chars.LF;
					Iterator = 1;
