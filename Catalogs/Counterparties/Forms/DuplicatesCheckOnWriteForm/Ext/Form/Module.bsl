...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	InscriptionMessage = NStr("ru = 'В справочнике ""Контрагенты"" найдены дублирующие элементы.
	|Отменить запись текущей номенклатуры, заменив контрагента, заменив его контрагентом из списка?'; en = 'The catalog "" Counterparties "" found duplicate elements.
	|To cancel recording the current element, replacing it with the counterparty from the list?'"); 
	...
	ListOfCounterparties.Parameters.SetParameterValue("ArrayOfRef", ArrayOfRef);
	
	For each ItemFound In Parameters.FoundObjects Do
		SetListAppearance(ItemFound.Value, ListOfCounterparties.ConditionalAppearance);
	EndDo;
...
Procedure SetListAppearance(ItemFound, Val ConditionalAppearance)
	...
	If ItemFound.Свойство("TIN_Flag") And ItemFound.TIN_Flag Then
		NewElement = ConditionalAppearanceItem.Fields.Items.Add();
		NewElement.Field = New DataCompositionField("TIN");		
	EndIf;
	
	If ItemFound.Свойство("KPP_Flag") And ItemFound.KPP_Flag Then
		NewElement = ConditionalAppearanceItem.Fields.Items.Add();
		NewElement.Field = New DataCompositionField("KPP");		
	EndIf;
...
Procedure Yes(Command)
	...
	If Items.ListOfCounterparties.CurrentData <> Undefined Then
		Close(Items.ListOfCounterparties.CurrentData.Ref);
	EndIf;
