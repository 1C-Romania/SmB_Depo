...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CancelPersonnelGLAccountChange(Ref) Then
		Items.WithStaff.ToolTip = NStr("en = 'There are records in the base for this employee! Change of settlement GL accounts with human resources is prohibited!'");
		Items.WithStaff.Enabled = False;
	EndIf;

	If CancelAdvanceHoldersGLAccountChange(Ref) Then
		Items.WithAdvanceHolder.ToolTip = NStr("en = 'There are records in the base for this advance holder! Change of settlement GL accounts with advance holders is prohibited!'");
