...
Procedure FillByOrder(Command)
	...
	ShowQueryBox(New NotifyDescription("FillEndByOrder", ThisObject), NStr("en = 'The document will be completely refilled according to ""Customer order""! Continue?'"), QuestionDialogMode.YesNo, 0);
...
Procedure CustomerOrderOnChange(Item)
	...
	If Object.Prepayment.Count() > 0
	   ...
	   AND CounterpartyDoSettlementsByOrders Then
		...
		ShowQueryBox(New NotifyDescription("CustomerOrderOnChangeEnd", ThisObject), NStr("en = 'Prepayment set-off will be cleared, do you want to continue?'"), Mode, 0);
        Return;
	EndIf;
