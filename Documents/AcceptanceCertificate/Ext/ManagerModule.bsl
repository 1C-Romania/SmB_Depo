...
Procedure GenerateTableInventory(DocumentRefAcceptanceCertificate, StructureAdditionalProperties)
	...
	For n = 0 To StructureAdditionalProperties.TableForRegisterRecords.TableInventory.Count() - 1 Do
		...
		If QuantityWanted > 0 Then
			...
			TableRowExpense.ContentOfAccountingRecord = NStr("en = 'Inventory write off'");
			...
			If Round(AmountToBeWrittenOff, 2, 1) <> 0 Then
				...
				RowTableManagerial.Content = NStr("en = 'Inventory write off'");
...
//
Procedure GenerateUniversalTransferDocument(ObjectsArray, PrintObjects, SpreadsheetDocument)
	...
	SpreadsheetDocument.PrintParametersKey = "PrintParameters_UniversalTransferDocument";
	...
	|	AcceptanceCertificate.Number AS Number,
	|	AcceptanceCertificate.Date AS DocumentDate,
	|	AcceptanceCertificate.Company AS Vendor,
	|	AcceptanceCertificate.Company AS Heads,
	|	AcceptanceCertificate.Counterparty AS Customer,
	|	ISNULL(AcceptanceCertificate.Contract.SettlementsInStandardUnits, FALSE) AS SettlementsInStandardUnits,
	|	AcceptanceCertificate.AmountIncludesVAT AS AmountIncludesVAT,
	|	AcceptanceCertificate.DocumentCurrency AS Currency,
	|	AcceptanceCertificate.Company.Prefix AS Prefix,
	|	AcceptanceCertificate.WorksAndServices.(
	|		LineNumber AS LineNumber,
	|		CASE
	...
	While Header.Next() Do
		...
		TabularSection				= Header.TabularSection.Unload();
		...
		FillStructureSection.Insert("PresentationOfShipper", "--");
		FillStructureSection.Insert("PresentationOfConsignee", "--");
		...
		For Each Row IN TabularSection Do
			...
			If IsBlankString(ProductDescription) Then
				...
				ProductDescription = SmallBusinessServer.GetProductsAndServicesPresentationForPrinting(Row.ProductDescription, Row.Characteristic, );
				
			EndIf;
			FillStructureSection.Insert("ProductDescription", ProductDescription);
			
			FillStructureSection.Insert("MeasurementUnit", "--");
			FillStructureSection.Insert("MeasurementUnitCode", "--");
			FillStructureSection.Insert("Excise", NStr("en = 'no excise'"));
			FillStructureSection.Insert("Quantity", "--");
...
EndProcedure // GenerateUniversalTransferDocument()
...
Function PrintForm(ObjectsArray, PrintObjects, TemplateName) Export
	...
	ElsIf TemplateName = "UniversalTransferDocument" Then
		...
		GenerateUniversalTransferDocument(ObjectsArray, PrintObjects, SpreadsheetDocument);
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "UniversalTransferDocument") Then
		...
		PrintManagement.OutputSpreadsheetDocumentToCollection(PrintFormsCollection, "UniversalTransferDocument", "Services acceptance certificate", PrintForm(ObjectsArray, PrintObjects, "UniversalTransferDocument"));
