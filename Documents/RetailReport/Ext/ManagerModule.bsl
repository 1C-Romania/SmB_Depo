...
// Function generates tabular document of petty cash book cover.
//
Function GeneratePrintFormKM6(ObjectsArray, PrintObjects)
	
	Template = PrintManagement.PrintedFormsTemplate("Document.RetailReport.PF_MXL_KM6");
	
	Spreadsheet = New SpreadsheetDocument;
	Spreadsheet.PrintParametersName = "PARAMETERS_PRINT_KM6";
	
	FirstDocument = True;
	
	For Each RetailReport IN ObjectsArray Do
		...
		QueryText =
		"SELECT
		|	ISNULL(SUM(TSInventory.Amount), 0) AS Amount,
		|	RetailReport.CashCR AS CashCR,
		|	RetailReport.CashCR.Peripherals.Description AS CashRegisterPresentation,
		|	PRESENTATION(RetailReport.StructuralUnit) AS StructuralUnitPresentation,
		|	RetailReport.Company AS Company,
		|	RetailReport.Company.Prefix AS Prefix,
		|	RetailReport.Number AS Number,
		|	RetailReport.Date AS DocumentDate,
		|	RetailReport.DocumentCurrency AS Currency,
		|	RetailReport.CashCRSessionStart AS BeginningOfChange,
		|	RetailReport.CashCRSessionEnd AS SessionEnd
		|FROM
		|	Document.RetailReport AS RetailReport
		|		LEFT JOIN Document.ReceiptCR.Inventory AS TSInventory
		|		ON (TSInventory.Ref.CashCRSession = RetailReport.Ref)
		|WHERE
		|	RetailReport.Ref = &RetailReport
		|
		|GROUP BY
		|	RetailReport.Company,
		|	RetailReport.Number,
		|	RetailReport.Date,
		|	RetailReport.DocumentCurrency,
		|	RetailReport.CashCRSessionStart,
		|	RetailReport.CashCRSessionEnd,
		|	RetailReport.CashCR,
		|	RetailReport.Company.Prefix,
		|	PRESENTATION(RetailReport.StructuralUnit)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|SELECT
		...
		|	RetailReport.Company AS Company,
		|	RetailReport.Company.Prefix AS Prefix,
		|	RetailReport.Number AS DocumentNumber,
		|	RetailReport.Date AS DocumentDate,
		|	RetailReport.DocumentCurrency AS Currency,
		|	RetailReport.CashCRSessionStart AS BeginningOfChange,
		|	RetailReport.CashCRSessionEnd AS SessionEnd
		|FROM
		|	Document.RetailReport AS RetailReport
		|		LEFT JOIN Document.ReceiptCRReturn.Inventory AS TSInventory
		|		ON (TSInventory.Ref.CashCRSession = RetailReport.Ref)
		|WHERE
		|	RetailReport.Ref = &RetailReport
		|
		...
		Query.SetParameter("RetailReport", RetailReport);
		Result = Query.ExecuteBatch();
		
		Sales = Result[0].Select();
		Sales.Next();
		
		Refunds = Result[1].Select();
		Refunds.Next();
		
		InfoAboutCustomer = SmallBusinessServer.InfoAboutLegalEntityIndividual(Sales.Company, Sales.DocumentDate, ,);
		
		If Sales.DocumentDate < Date('20110101') Then
			DocumentNumber = SmallBusinessServer.GetNumberForPrinting(Sales.Number, Sales.Prefix);
		Else
			DocumentNumber = ObjectPrefixationClientServer.GetNumberForPrinting(Sales.Number, True, True);
		EndIf;
			
		TemplateArea = Template.GetArea("Title");
		TemplateArea.Parameters.Fill(Sales);
		TemplateArea.Parameters.CompanyPresentation	= SmallBusinessServer.CompaniesDescriptionFull(InfoAboutCustomer, "FullDescr,LegalAddress,PhoneNumbers,");
		TemplateArea.Parameters.DocumentDate				= Sales.DocumentDate;
		TemplateArea.Parameters.CompanyByOKPO			= InfoAboutCustomer.CodeByOKPO;
		TemplateArea.Parameters.CompanyTIN				= InfoAboutCustomer.TIN;
		TemplateArea.Parameters.AccountingApplication				= "1C:Enterprise 8";
		...
		TemplateArea.Parameters.Fill(Sales);
		TemplateArea.Parameters.Total = Sales.Amount;
		TemplateArea.Parameters.TotalReturn = Refunds.Amount;
		Spreadsheet.Put(TemplateArea);
		...
		If ValueIsFilled(Sales.Amount) Then
			TemplateArea.Parameters.AmountOfRevenueInWords = WorkWithCurrencyRates.GenerateAmountInWords(
				Sales.Amount - Refunds.Amount,
				Sales.Currency
			);
		EndIf;
		...
		TemplateArea = Template.GetArea("Turnover");
		
		Heads = SmallBusinessServer.OrganizationalUnitsResponsiblePersons(Sales.Company, Sales.DocumentDate);
		...
		TemplateArea = Template.GetArea("PrintWithTurnover");
		Spreadsheet.Put(TemplateArea);
		
		PrintManagement.SetDocumentPrintArea(Spreadsheet, FirstLineNumber, PrintObjects, RetailReport);
...
EndFunction // GenerateKM6PrintingForm()
...
Function GeneratePrintFormKM3(ObjectsArray, PrintObjects)
	...
	Template = PrintManagement.PrintedFormsTemplate("Document.RetailReport.PF_MXL_KM3");
	...
	|	Doc.CashCR AS CashCR,
	|	Doc.CashCR.CashCRType AS CashCRType,
	|	Doc.CashCR.Presentation AS Customer,
	|	Doc.Author AS Author,
	|	Doc.DocumentCurrency AS Currency,
	|	Doc.Company AS Company,
	|	Doc.Company.Presentation AS Vendor,
	|	"""" AS CashRegisterSerialNumber,
	|	"""" AS CashRegisterRegistrationNumber,
	|	"""" AS CashRegisterPresentation
	|FROM
	|	Document.RetailReport AS Doc
	|WHERE
	|	Doc.Ref IN(&ObjectsArray)
	|ORDER BY
	|	Ref
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	|	DocRetailSalesReport.Ref AS RetailReport,
	|	DocReceipt.ReceiptCRNumber               AS ReceiptNumber,
	|	DocReceipt.DocumentAmount             AS DocumentAmount
	|FROM
	|	Document.RetailReport AS DocRetailSalesReport
	|		INNER JOIN Document.ReceiptCRReturn AS DocReceipt
	|		ON DocRetailSalesReport.Ref = DocReceipt.CashCRSession
	|			AND (DocReceipt.Posted)
	|			AND (DocReceipt.ReceiptCRNumber > 0)
	|WHERE
	|	DocRetailSalesReport.Ref IN(&ObjectsArray)
	|ORDER BY
	|	RetailReport
	|TOTALS BY
	|	DocRetailSalesReport.Ref
	|";
	...
	While Selection.Next() Do
		...
		SelectionByReceiptsTotals = QueryResult[1].Select(QueryResultIteration.ByGroups);
		If SelectionByReceiptsTotals.FindNext(New Structure("RetailReport", Selection.Ref)) Then
			...
			Heads          = SmallBusinessServer.OrganizationalUnitsResponsiblePersons(Selection.Company, Selection.DocumentDate);
			
			HeaderArea = Template.GetArea("Header");
			HeaderArea.Parameters.Fill(Selection);
			InfoAboutCompany							= SmallBusinessServer.InfoAboutLegalEntityIndividual(Selection.Company, Selection.DocumentDate, ,);
			HeaderArea.Parameters.CompanyPresentation	= SmallBusinessServer.CompaniesDescriptionFull(InfoAboutCompany, "FullDescr,TIN,KPP,PhoneNumbers,");;
			HeaderArea.Parameters.DocumentNumber			= "";
			HeaderArea.Parameters.CompanyByOKPO		= InfoAboutCompany.CodeByOKPO;
			HeaderArea.Parameters.CompanyTIN			= InfoAboutCompany.TIN;
			HeaderArea.Parameters.ActivityKindByOKDP	= "";
			HeaderArea.Parameters.HeadPost	= Heads.HeadPosition;
			Spreadsheet.Put(HeaderArea);
			
			HeadDescriptionFull = Heads.HeadDescriptionFull;
			
			AmountTotal     = 0;
			SerialNumber = 0;
			SelectionByReceipts = SelectionByReceiptsTotals.Select();
			While SelectionByReceipts.Next() Do
				
				SerialNumber = SerialNumber + 1;
				RowArea = Template.GetArea("String");
				RowArea.Parameters.Fill(SelectionByReceipts);
				RowArea.Parameters.HeadDescriptionFull = HeadDescriptionFull;
				RowArea.Parameters.Number = SerialNumber;
				Spreadsheet.Put(RowArea);
				
				AmountTotal = AmountTotal + SelectionByReceipts.DocumentAmount;
				
			EndDo;
			
			FooterArea = Template.GetArea("Footer");
			
			If ValueIsFilled(Selection.Author) Then
				CashierOperatorNameAndSurname = Selection.Author;
			EndIf;
			
			SeniorCashier         = Heads.CashierNameAndSurname;
			
			FooterArea.Parameters.DocumentAmount          = AmountTotal;
			FooterArea.Parameters.AmountInWords           = WorkWithCurrencyRates.GenerateAmountInWords(AmountTotal, Selection.Currency);
			FooterArea.Parameters.CompanyCashierNameAndSurname   = SeniorCashier;
			FooterArea.Parameters.CashierOperatorNameAndSurname = CashierOperatorNameAndSurname;
			Spreadsheet.Put(FooterArea);
			
			PrintManagement.SetDocumentPrintArea(Spreadsheet, FirstLineNumber, PrintObjects, Selection.Ref);
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	ElsIf PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "KM6") Then
		
		PrintManagement.OutputSpreadsheetDocumentToCollection(
			PrintFormsCollection,
			"KM6",
			"Report for shift",
			GeneratePrintFormKM6(ObjectsArray, PrintObjects)
		);
		
	ElsIf PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "KM3") Then
		
		PrintManagement.OutputSpreadsheetDocumentToCollection(
			PrintFormsCollection,
			"KM3",
			"KM-3",
			GeneratePrintFormKM3(ObjectsArray, PrintObjects)
		);
