...
Function ShowUserAlertAboutPossibleError(Text, Quantity, CountTotal)
	...
	If Quantity < CountTotal Then
		...
		Text = Text
		        + Chars.LF
		        + NStr("en = 'Check:either the quantity is not filled or the price tag (label) template is not specified. IN this case the selection mark for strings can not be specified.'");
...
Procedure FillInventoryTable(Command)
	...
	QuestionText = NStr("en = 'All manually entered data will be lost. Do you want to continue?'");
...
Procedure Print(Command)
	...
	If SelectedRows.Count() = 0 Then
		...
		ShowMessageBox(Undefined,NStr("en = 'No items selected'"));
		Return;
	...
	If ThisForm.CheckFilling() Then
		...
		PrintParameters.Insert("FormTitle", NStr("en = 'Print labels and tags'"));
	...
	If CurrentData.Selected Then
		...
		If Mode = "TagsAndLabePrinting" Then
			...
			If (ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message.Text = NStr("en = 'The price tags number is not filled'");
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en = 'The labels number is not filled'");
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en = 'No label template selected'");
			...
			ElsIf (   ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en = 'Price tags and (or) labels quantity is not filled'");
				Message.Field = "Object.Products["+CurrentRow+"].PriceTagsQuantity";
				Message.Message();
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en = 'Price tags and(or) labels templates are not selected'");
				Message.Field = "Object.Products["+CurrentRow+"].PriceTagsTemplate";
				Message.Message();
			...
			ElsIf (   ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en = 'The number of price tags and (or) the label template is not filled'");
				Message.Field = "Object.Products["+CurrentRow+"].PriceTagsQuantity";
				Message.Message();
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en = 'The number of labels and (or) the price tag template is not filled'");
				Message.Field = "Object.Products["+CurrentRow+"].LabelsQuantity";
				Message.Message();
...
Procedure SetPriceTagsQuantity(Command)
	...
	ShowInputNumber(New NotifyDescription("SetPriceTagsQuantityEnd", ThisObject, New Structure("ValueSelected", ValueSelected)), ValueSelected, NStr("en = 'Enter the number of the price tags'"), 10, 2);
...
Procedure SetPriceTagsQuantityEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en = 'Price tags quantity %Quantity% specified.'");
...
Procedure SetLabelsQuantityEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en = 'Labels  quantity %Quantity% specified.'");
        Text = StrReplace(Text, "%Quantity%", ValueSelected);
...
Procedure SetPriceTagsTemplateEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en = 'The %Template% template specified.'");
