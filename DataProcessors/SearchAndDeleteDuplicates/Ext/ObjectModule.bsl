...
Function DuplicateSearchAreaManager(Val DataSearchAreaName) Export
	...
	Raise StrReplace(
		NStr("en = 'Unknown type of metadata object ""%1""'"), 
	...
	While Selection.Next() Do
		...
		If ReturnPortionSize > 0 AND (DuplicatesTable.Count() > ReturnPortionSize) Then
			...
			If DuplicatesTable.Count() = 0 Then
				Result.ErrorDescription = NStr("en = 'Too many items are found, not all duplicates groups are defined.'");
			Else
				Result.ErrorDescription = NStr("en = 'Too many items are found. Specify criteria of duplicates search.'");
			EndIf;
			Break;
		EndIf;
...
Procedure RunCommand(ExecuteParameters, ResultAddress) Export
	...
	Else
		...
		Raise NStr("en = 'Search and delete duplicates: %1 command is not supported.'");
