...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Else
			Raise StrReplace(
				NStr("en = 'Incorrect object parameters of assigning the command opening ""%1""'"),
	...
	If Not MonitorSettings() AND OpenNodeParameter Then
		...
		MessageText = StrReplace(
			NStr("en = 'For ""%1"" objects registration editing is unavailable.'"),
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	Then
		Error = NStr("en = 'Unexpected result of selection from the request console'");
	Else
	...
	If ValueSelected.ActionSelect Then
		Text = NStr("en = 'Register query
		                 |result on the node ""%1""?'"); 
	Else
		Text = NStr("en = 'Cancel query result
		                 |registration on the node ""%1""?'");
	EndIf;
...
Procedure ShowExportResult(Command)
	...
	If Serialization.Count() > 0 Then
		...
		TitleText = NStr("en = 'Standard export result (RIB)'");
		Text.Show(TitleText);
	EndIf;
...
Procedure RegisterIOMAndPredetermined(Command)
	...
	QuestionText     = StrReplace( 
		NStr("en = 'Register data to restore DIB
		     |subordinate node on the node ""%1""?'"),
		"%1", ExchangeNodeRef
	);
...
Procedure SetTitleNumbersMessages()
	...
	Text = NStr("en = '%1 sent No, %2 receved No'");
...
Procedure MessageAboutResultsOfRegistration(Command, Results)
	...
	If TypeOf(Command) = Type("Boolean") Then
		If Command Then
			TitleWarnings = NStr("en = 'Change records:'");
			WarningText = NStr("en = '%1 changes are registered
			                           |from %2 on the node ""%0""'");
		Else
			TitleWarnings = NStr("en = 'Registration cancel:'");
			WarningText = NStr("en = '%1 changes registration is canceled on the node ""%0"".'");
		EndIf;
	Else
		TitleWarnings = NStr("en = 'The message number change:'");
		WarningText = NStr("en = 'Message number is changed
		                           |to %3 in the %1 object(s)'");
	EndIf;
...
Procedure DeleteConstantRegistrationInList()
	...
	Else
		Text = NStr("en = 'Cancel registration of
		                 |the selected constants on the node ""%1""?'"); 
	EndIf;
...
Procedure DeleteRegistrationFromRefsList()
	...
	Else
		Text = NStr("en = 'Cancel registration of
		                 |the selected objects on the node ""%1""?'"); 
	EndIf;
	Text = StrReplace(Text, "%1", ExchangeNodeRef);
...
Procedure AddSelectedObjectsRegistration(WithoutAccountingAutoRecord = True)
	...
	Text = NStr("en = 'Register %1 for export
	                 |
	                 |on the node ""%2""? Changing the registration of many objects may take a long time.'");
...
Procedure OperationWithResultsOfQuery(CommandOperations)
	...
	If CurFormName <> Undefined Then
		...
		If CommandOperations Then
			Text = NStr("en = 'Request result modifications registration'");
		Else
			Text = NStr("en = 'Cancel the modifications registration of the request result'");
		EndIf;
...
Function QuerySelectControlLinks(Address)
	...
	If TypeOf(Result) = Type("Array") Then 
		...
		EndIf;
	Else		
		Return NStr("en = 'Error when receiving the request result data'");
	EndIf;
...
Procedure CustomizeBlankPage(Description, TableName = Undefined)
	...
	Text = NStr("en = '%1.
	                 ...
	                 |left in the metadata tree and use the commands ""Register"" or ""Cancel registration""'");
...
Function GetDescriptionSelectedMetadata(WithoutAccountingAutoRecord, MetaNameGroup = Undefined, MetaNameNode = Undefined)
	...
	If MetaNameGroup = Undefined AND MetaNameNode = Undefined Then
		...
		Text = NStr("en = 'all objects %1 by the selected type hierarchy'");
	...
	Else
		...
		Text = NStr("en = 'all objects of the type ""%3"" %1'");
	...
	If WithoutAccountingAutoRecord Then
		FlagText = "";
	Else
		FlagText = NStr("en = 'with autoregistration sign'");
	EndIf;
...
Procedure HandleProhibitionChangesNode()
	...
	If OperationsAllowed Then
		...
		Title = NStr("en = 'Registration of modifications for the data exchange'");
	Else
		Items.ExchangeNodeRef.Visible = False;
		Title = StrReplace(NStr("en = 'Changes registration for the exchange with ""%1""'"), "%1", String(ExchangeNodeRef));
	EndIf;
...
Function ValidateVersionAndPlatformCompatibilityMode()
	...
	If Not (Left(Information.AppVersion, 3) = "8.3"
		...
		Raise NStr("en = 'Data processor is designed to start
			|on the 1C:Enterprise 8.3.5 platform version with the disabled compatibility mode or higher'");
