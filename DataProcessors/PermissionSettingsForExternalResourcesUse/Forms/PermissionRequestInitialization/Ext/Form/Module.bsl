...
Function StartRequestsProcessing(Val Queries, Val ConnectMode, DisconnectMode, Val RecoveryMode, Val UsageCheckMode)
	...
	Task = BackgroundJobs.Execute("WorkInSafeMode.ExecuteConfigurationMethod",
			...
			,
			NStr("en = 'Processing requests to use external resources...'"));
...
Function RequestsProcessed(Val JobID)
	...
	If Task = Undefined Then
		Raise(NStr("en = 'An error occurred while processing requests. A job to process the requests is not found.'"));
	EndIf;
	...
	If Task.State = BackgroundJobState.Failed Then
		...
		Else
			Raise(NStr("en = 'An error occurred while processing requests. A job to process the requests is terminated with an unknown error.'"));
		EndIf;
	ElsIf Task.State = BackgroundJobState.Canceled Then
		Raise(NStr("en = 'An error occurred while processing requests. A job to process the requests is terminated by administrator.'"));
	Else
		JobID = Undefined;
		Return True;
	EndIf;
...
Procedure CancelRequestsProcessing(Val JobID)
	...
	Except
		...
		WriteLogEvent(NStr("en = 'Configure permissions to use external resources. Cancel background job'", CommonUseClientServer.MainLanguageCode()),
