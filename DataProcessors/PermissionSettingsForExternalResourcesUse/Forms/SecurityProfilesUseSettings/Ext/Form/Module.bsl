...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not WorkInSafeModeService.SecurityProfilesCanBeUsed() Then
		Raise NStr("en = 'Security profiles use is unavailable for this configuration.'");
	EndIf;
	
	If Not WorkInSafeModeService.SecurityProfilesSetupAvailable() Then
		Raise NStr("en = 'Security profile customization is unavailable.'");
	EndIf;
	
	If Not RunMode.ThisIsSystemAdministrator Then
		Raise NStr("en = 'You have no enough access rights!'");
	EndIf;
...
Procedure StartUseSecurityProfilesUseSettings(Val UUID)
	...
	If Not WorkInSafeModeService.SecurityProfilesCanBeUsed() Then
		Raise NStr("en = 'Enabling of automatic permissions requests is unavailable.'");
	EndIf;
