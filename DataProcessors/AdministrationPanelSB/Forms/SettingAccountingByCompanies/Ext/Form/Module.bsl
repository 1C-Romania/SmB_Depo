	...
	While SelectionCompanies.Next() Do
		...
		If SelectionCompanies.Ref <> MainCompany Then
			...
			If RefsTable.Count() > 0 Then
				...
				MessageText = NStr("en = 'Companies that differ from the main one are used in the base! Disabling the option is prohibited!'");
...
Function CancellationSetAccountingBySubsidiaryCompanyChangeSubsidiaryCompany(FieldName)
	...
	If AreRecords Then
		MessageText = NStr("en = 'There are records of an organization other than the company in the base! Parameter change is prohibited!'");
...
Function CancellationUncheckAccountingBySubsidiaryCompany()
	...
	If AreDocuments Then
		MessageText = NStr("en = 'In the base there are posted documents from an organization other than company! You can not clear the ""Accounting by company"" check box!'");	
...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If ConstantsSet.AccountingBySubsidiaryCompany 
		...
		MessageText = NStr("en = 'The ""Account by company"" check box is selected, but the ""Organization-company"" is not filled!'");
