...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If TypeOf(Parameters.Filter) = Type("Structure") Then
		...
		For Each FilterItemFromParameters IN Parameters.Filter Do
			...
			If TypeOf(FilterItemFromParameters.Value) = Type("Array") Then
				If FilterItemFromParameters.Value.Count() = 1 Then
					FilterValue = FilterItemFromParameters.Value[0];
				Else
					FilterValue = New ValueList;
					FilterValue.LoadValues(FilterItemFromParameters.Value);
				EndIf;
			Else
				FilterValue = FilterItemFromParameters.Value;
			EndIf;
			
			CompositionComparisonType = ?(TypeOf(FilterValue) = Type("ValueList"), DataCompositionComparisonType.InList, DataCompositionComparisonType.Equal); 
