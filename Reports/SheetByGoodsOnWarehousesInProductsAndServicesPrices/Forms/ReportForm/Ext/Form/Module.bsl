...
// identifier ParameterValue - parameter value for record
//
&AtServer
...
//
// CompositionFilterCorrespondence - compliance, contains setting filter items of data composition and its identifiers
//  FilterItemFromParameters - form parameter structure item, contains key and value filter items 
// UserSettingFilter - item collection of user setting filter 
// FilterValue - filter value 
// CompositionComparisonKind - comparsion kind of data composition
// Usage - filter use value of data composition
//
&AtServer
Procedure SetDataCompositionFilterItemAtServer(CompositionFilterCorrespondence, DataCompositionFieldName, UserSettingsFilter, FilterValue, CompositionComparisonType, Use)
	
	CompositionFilterNewField	= New DataCompositionField(DataCompositionFieldName);
...
// Procedure checks if there is an option to detail the data.
//
&AtServer
Procedure ProcessorDecryptionAtServer(Details, StandardProcessing)
	
	DetailsCollection = GetFromTempStorage(DetailsData);
	If TypeOf(DetailsCollection) = Type("DataCompositionDetailsData") Then
		
		FieldCollection = DetailsCollection.Items[Details].GetFields();
		RegisterField = FieldCollection.Find("Recorder");
		If RegisterField <> Undefined
			AND RegisterField.Value = Undefined Then
...
Procedure SetPeriod(PeriodName, Direction)
	...
	Result.Area(2,2,2,2).Text 		= NStr("en = 'Report is not generated. Click Create to generate the report.'");
...
////////////////////////////////////////////////////////////////////////////////
// PROCEDURE - FORM AND ATTRIBUTES EVENT HANDLERS
...
//Write the value of the saved form attribute to the parameter 
//
&AtServer
Procedure OnLoadDataFromSettingsAtServer(Settings)
	
	SetParameterAtServer("PriceKind", Report.PriceKind);
...
Procedure SettingsComposerUserSettingsOnChange(Item)
	...
	If Not DataCompositionObject = Undefined
		...
		If DataCompositionObject.Parameter = New DataCompositionParameter("PriceKind") Then
			
			Report.PriceKind = DataCompositionObject.Value;
			Return;
...
// Procedure - event handler OnChange field PriceKind
//
&AtClient
